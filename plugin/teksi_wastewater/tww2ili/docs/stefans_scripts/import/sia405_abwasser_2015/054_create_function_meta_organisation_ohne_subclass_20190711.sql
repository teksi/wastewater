...
-- 3.6.2017 / 11.7.2019 Stefan Burckhardt

-- Adds organisations that do not exist yet (e.g. because they are used as dataowner/datenherr or provider/datenlieferant) in class organisation and sublcass privat
-- e.g. SELECT abwa_2015neu_3122.meta_organisation('VSA', 'ch080qwzOG000098');

-- DROP FUNCTION abwa_2015neu_3122.meta_organisation(bez text, objid text)

CREATE OR REPLACE FUNCTION abwa_2015neu_3122.meta_organisation(bez text, objid text)
  RETURNS integer AS
$BODY$
DECLARE
...
BEGIN
  ...
  IF NOT FOUND THEN
	...
	RAISE NOTICE 'New data in Organisation added wit obj_id %', objid;
...
ALTER FUNCTION abwa_2015neu_3122.meta_organisation(text, text)
  OWNER TO postgres;
