...
class InterlisTools:
    ...
    def validate_xtf_data(self, xtf_file, log_path):
        ...
        execute_subprocess(
            f'"{self.java_executable_path}" -jar "{config.ILIVALIDATOR}" --model_dir http://https://vsa.ch/models/;%ITF_DIR;http://models.interlis.ch/;%JAR_DIR/ilimodels --log "{log_path}" "{xtf_file}"'
        )
