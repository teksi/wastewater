...
class ExtractlabelsInterlisAlgorithm(TwwAlgorithm):
    ...
    INPUT_MEASURE_LAYER = "MEASURE_LAYER"
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterVectorLayer(
                self.INPUT_MEASURE_LAYER,
                description=self.tr("Measure layer"),
                types=[QgsWkbTypes.PointGeometry],
            )
        )
    ...
    def processAlgorithm(
        ...
        measure_layer = self.parameterAsVectorLayer(
            parameters, self.INPUT_MEASURE_LAYER, context
        )
        ...
        if restrict_to_selection:
            ...
            extent.combineExtentWith(catchment_layer.boundingBoxOfSelected())
            extent.combineExtentWith(measure_layer.boundingBoxOfSelected())
            extent.combineExtentWith(building_group_layer.boundingBoxOfSelected())
        else:
        ...
        measure_feats = measure_layer.getFeatures()
        ...
        rowid_to_obj_id = {
            ...
            "vw_einzugsgebiet_ag96": {
                f.id(): f.attribute("obj_id") for f in catchment_feats
            },
            "vw_massnahme_ag96": {
                f.id(): f.attribute("obj_id") for f in measure_feats
            },
            "vw_bautenausserhalbbaugebiet_ag96": {
        ...
        for scale_id, scale_display, scale_value in scales:
            ...
            lyr_name_to_key = {
                ...
                TwwLayerManager.layer("vw_einzugsgebiet_ag96").name(): "vw_einzugsgebiet_ag96",
                TwwLayerManager.layer("vw_massnahme_ag96").name(): "vw_massnahme_ag96",
                TwwLayerManager.layer("vw_bautenausserhalbbaugebiet_ag96").name(): "vw_bautenausserhalbbaugebiet_ag96",
            }
