...
def test_import_orgs(clean_db):
    run_cli(
        "interlis_import "
        "--xtf_file /usr/src/plugin/teksi_wastewater/tests/data/minimal-dataset-organisation-arbon-only.xtf "
...
def test_import_sia405(clean_db):
    run_cli(
        "interlis_import "
        "--xtf_file /usr/src/plugin/teksi_wastewater/tests/data/minimal-dataset-SIA405-ABWASSER.xtf "
        "--pghost db --pgdatabase tww --pguser postgres --pgpass postgres --pgport 5432"
    )
