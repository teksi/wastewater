...
When updating a reach, there are several cases:
  ...
  1. The first/last reach geometry vertex was not changed, but the `rp_from_level` / `rp_to_level` was. In this case, the level value `rp_from_level` / `rp_to_level` is used to set the Z value of the first / last vertex of the reach
