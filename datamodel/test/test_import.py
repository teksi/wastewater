...
class TestImport(unittest.TestCase, DbTestBase):
    ...
    def test_update_with_outlet(self):
        ...
        cur.execute(
            ...
            ).format(schema=psycopg3.sql.Identifier("tww_od")),
            {"obj_id": obj_id},
        )
        row = cur.fetchone()
        self.assertIsNotNone(row)
    ...
    def test_general(self):
        ...
        cur = self.cursor()
        cur.execute(
            psycopg3.sql.SQL("SELECT * FROM {schema}.wastewater_structure LIMIT 1").format(
                schema=psycopg3.sql.Identifier("tww_od")
            )
        )
        row = cur.fetchone()
        self.assertIsNotNone(row)
