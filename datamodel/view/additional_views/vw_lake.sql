DROP VIEW IF EXISTS tww_od.vw_lake;
...
--------
CREATE OR REPLACE VIEW tww_od.vw_lake AS
...
CREATE OR REPLACE FUNCTION tww_od.vw_lake_insert()
  RETURNS trigger AS
$BODY$
BEGIN
  INSERT INTO tww_od.surface_water_bodies (
     ...
     VALUES ( COALESCE(NEW.obj_id,tww_sys.generate_oid('tww_od','lake')) -- obj_id
           , NEW.identifier
           , NEW.remark
           , NEW.fk_dataowner
           , NEW.fk_provider
...
CREATE TRIGGER vw_lake_ON_INSERT INSTEAD OF INSERT ON tww_od.vw_lake
  FOR EACH ROW EXECUTE PROCEDURE tww_od.vw_lake_insert();
...
CREATE OR REPLACE RULE vw_lake_ON_UPDATE AS ON UPDATE TO tww_od.vw_lake DO INSTEAD (
UPDATE tww_od.lake
  SET
...
CREATE OR REPLACE RULE vw_lake_ON_DELETE AS ON DELETE TO tww_od.vw_lake DO INSTEAD (
