DROP VIEW IF EXISTS tww_od.vw_lock;
...
--------
CREATE OR REPLACE VIEW tww_od.vw_lock AS
...
CREATE OR REPLACE FUNCTION tww_od.vw_lock_insert()
  RETURNS trigger AS
$BODY$
BEGIN
...
CREATE TRIGGER vw_lock_ON_INSERT INSTEAD OF INSERT ON tww_od.vw_lock
  FOR EACH ROW EXECUTE PROCEDURE tww_od.vw_lock_insert();
...
CREATE OR REPLACE RULE vw_lock_ON_UPDATE AS ON UPDATE TO tww_od.vw_lock DO INSTEAD (
UPDATE tww_od.lock
  SET
...
CREATE OR REPLACE RULE vw_lock_ON_DELETE AS ON DELETE TO tww_od.vw_lock DO INSTEAD (
