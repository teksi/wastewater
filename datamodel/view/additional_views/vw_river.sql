DROP VIEW IF EXISTS tww_od.vw_river;
...
--------
CREATE OR REPLACE VIEW tww_od.vw_river AS
...
CREATE OR REPLACE FUNCTION tww_od.vw_river_insert()
  RETURNS trigger AS
$BODY$
BEGIN
...
CREATE TRIGGER vw_river_ON_INSERT INSTEAD OF INSERT ON tww_od.vw_river
  FOR EACH ROW EXECUTE PROCEDURE tww_od.vw_river_insert();
...
CREATE OR REPLACE RULE vw_river_ON_UPDATE AS ON UPDATE TO tww_od.vw_river DO INSTEAD (
UPDATE tww_od.river
  SET
       kind = NEW.kind
...
CREATE OR REPLACE RULE vw_river_ON_DELETE AS ON DELETE TO tww_od.vw_river DO INSTEAD (
