...
CREATE OR REPLACE VIEW tww_app.vw_catchment_area_wwc_connections AS
 SELECT ca.obj_id,
    st_force2d(st_makeline(st_centroid(st_curvetoline(ca.perimeter_geometry)), wn_ww_current.situation3d_geometry))::geometry(LineString, {SRID} ) AS connection_ww_current_geometry,
    st_length(st_force2d(st_makeline(st_centroid(st_curvetoline(ca.perimeter_geometry)), wn_ww_current.situation3d_geometry))::geometry(LineString, {SRID} )) AS length_ww_current,
