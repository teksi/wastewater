...
CREATE OR REPLACE VIEW tww_app.vw_catchment_area_rwp_connections AS
 SELECT ca.obj_id,
    ST_Force2D(st_makeline(st_centroid(st_curvetoline(ca.perimeter_geometry)), wn_rw_planned.situation3d_geometry))::geometry(LineString, {SRID} ) AS connection_rw_planned_geometry,
    ST_Length(ST_Force2D(st_makeline(st_centroid(st_curvetoline(ca.perimeter_geometry)), wn_rw_planned.situation3d_geometry))::geometry(LineString, {SRID} )) AS Length_rw_planned,
