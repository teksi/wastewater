DROP VIEW IF EXISTS tww_app.vw_chute;
...
--------
CREATE OR REPLACE VIEW tww_app.vw_chute AS
...
CREATE OR REPLACE FUNCTION tww_app.vw_chute_insert()
  RETURNS trigger AS
$BODY$
BEGIN
  INSERT INTO tww_od.water_control_structure (
     ...
     VALUES ( COALESCE(NEW.obj_id,tww_app.generate_oid('tww_od','chute')) -- obj_id
           , NEW.identifier
           , NEW.remark
...
CREATE TRIGGER vw_chute_ON_INSERT INSTEAD OF INSERT ON tww_app.vw_chute
  FOR EACH ROW EXECUTE PROCEDURE tww_app.vw_chute_insert();
...
CREATE OR REPLACE RULE vw_chute_ON_UPDATE AS ON UPDATE TO tww_app.vw_chute DO INSTEAD (
UPDATE tww_od.chute
  SET
       kind = NEW.kind
...
CREATE OR REPLACE RULE vw_chute_ON_DELETE AS ON DELETE TO tww_app.vw_chute DO INSTEAD (
