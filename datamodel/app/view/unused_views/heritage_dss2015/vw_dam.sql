DROP VIEW IF EXISTS tww_app.vw_dam;
...
--------
CREATE OR REPLACE VIEW tww_app.vw_dam AS
...
CREATE OR REPLACE FUNCTION tww_app.vw_dam_insert()
  RETURNS trigger AS
$BODY$
BEGIN
  INSERT INTO tww_od.water_control_structure (
     ...
     VALUES ( COALESCE(NEW.obj_id,tww_app.generate_oid('tww_od','dam')) -- obj_id
           , NEW.identifier
           , NEW.remark
...
CREATE TRIGGER vw_dam_ON_INSERT INSTEAD OF INSERT ON tww_app.vw_dam
  FOR EACH ROW EXECUTE PROCEDURE tww_app.vw_dam_insert();
...
CREATE OR REPLACE RULE vw_dam_ON_UPDATE AS ON UPDATE TO tww_app.vw_dam DO INSTEAD (
UPDATE tww_od.dam
  SET
       kind = NEW.kind
...
CREATE OR REPLACE RULE vw_dam_ON_DELETE AS ON DELETE TO tww_app.vw_dam DO INSTEAD (
