DROP VIEW IF EXISTS tww_app.vw_param_ca_general;
...
--------
CREATE OR REPLACE VIEW tww_app.vw_param_ca_general AS
...
SELECT
  ...
  FROM tww_od.param_ca_general PC
...
CREATE OR REPLACE FUNCTION tww_app.vw_param_ca_general_insert()
  RETURNS trigger AS
$BODY$
BEGIN
  INSERT INTO tww_od.surface_runoff_parameters (
     ...
     VALUES ( COALESCE(NEW.obj_id,tww_app.generate_oid('tww_od','param_ca_general')) -- obj_id
...
INSERT INTO tww_od.param_ca_general (
             obj_id
           , dry_wheather_flow
           , flow_path_length
           , flow_path_slope
...
-- DROP TRIGGER vw_param_ca_general_ON_INSERT ON tww_od.param_ca_general;

CREATE TRIGGER vw_param_ca_general_ON_INSERT INSTEAD OF INSERT ON tww_app.vw_param_ca_general
  FOR EACH ROW EXECUTE PROCEDURE tww_app.vw_param_ca_general_insert();
...
CREATE OR REPLACE RULE vw_param_ca_general_ON_UPDATE AS ON UPDATE TO tww_app.vw_param_ca_general DO INSTEAD (
UPDATE tww_od.param_ca_general
  SET
...
CREATE OR REPLACE RULE vw_param_ca_general_ON_DELETE AS ON DELETE TO tww_app.vw_param_ca_general DO INSTEAD (
