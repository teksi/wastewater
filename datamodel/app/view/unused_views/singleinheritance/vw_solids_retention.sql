DROP VIEW IF EXISTS tww_app.vw_solids_retention;
...
--------
CREATE OR REPLACE VIEW tww_app.vw_solids_retention AS
...
CREATE OR REPLACE FUNCTION tww_app.vw_solids_retention_insert()
  RETURNS trigger AS
$BODY$
BEGIN
  INSERT INTO tww_od.structure_part (
     ...
     VALUES ( COALESCE(NEW.obj_id,tww_app.generate_oid('tww_od','solids_retention')) -- obj_id
           , NEW.identifier
           , NEW.remark
...
CREATE TRIGGER vw_solids_retention_ON_INSERT INSTEAD OF INSERT ON tww_app.vw_solids_retention
  FOR EACH ROW EXECUTE PROCEDURE tww_app.vw_solids_retention_insert();
...
CREATE OR REPLACE RULE vw_solids_retention_ON_UPDATE AS ON UPDATE TO tww_app.vw_solids_retention DO INSTEAD (
...
CREATE OR REPLACE RULE vw_solids_retention_ON_DELETE AS ON DELETE TO tww_app.vw_solids_retention DO INSTEAD (
