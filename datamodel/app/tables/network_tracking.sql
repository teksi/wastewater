...
CREATE TABLE tww_app.network_node (
...
CREATE TABLE tww_app.network_segment (
  id SERIAL PRIMARY KEY,
  segment_type TEXT, -- either reach (if it's a reach segment) or junction (if it represents junction from/to a reachpoint)
  from_node INT REFERENCES tww_app.network_node(id) ON DELETE CASCADE,
  to_node INT REFERENCES tww_app.network_node(id) ON DELETE CASCADE,
