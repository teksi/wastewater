...
-------
CREATE TABLE qgep_od.re_building_group_disposal
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_re_building_group_disposal_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_re_building_group_disposal_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.re_building_group_disposal ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','re_building_group_disposal');
COMMENT ON COLUMN qgep_od.re_building_group_disposal.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
-------
CREATE TABLE qgep_od.re_maintenance_event_wastewater_structure
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_re_maintenance_event_wastewater_structure_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
...
COMMENT ON COLUMN qgep_od.re_maintenance_event_wastewater_structure.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
-------
CREATE TABLE qgep_od.txt_symbol
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_txt_symbol_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
...
COMMENT ON COLUMN qgep_od.txt_symbol.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.txt_symbol ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.txt_symbol.classname IS 'Name of class that symbol class is related to / Name der Klasse zu der die Symbolklasse gehört / nom de la classe à laquelle appartient la classe de symbole';
...
COMMENT ON COLUMN qgep_od.txt_symbol.symbolori IS 'Default: 90 Degree / Default: 90 Grad / Default: 90 degree';
ALTER TABLE qgep_od.txt_symbol ADD COLUMN symbolpos_geometry geometry('POINT', 2056);
...
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_txt_text_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
...
COMMENT ON COLUMN qgep_od.txt_text.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.txt_text ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.txt_text.classname IS 'Name of class that textclass is related to / Name der Klasse zu der die Textklasse gehört / nom de la classe à laquelle appartient la classe de texte';
...
ALTER TABLE qgep_od.txt_text ADD COLUMN textpos_geometry geometry('POINT', 2056);
...
-------
-------
CREATE TABLE qgep_od.progression_alternative
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_progression_alternative_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_progression_alternative_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.progression_alternative ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','progression_alternative');
COMMENT ON COLUMN qgep_od.progression_alternative.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.progression_alternative ADD COLUMN plantype  integer ;
COMMENT ON COLUMN qgep_od.progression_alternative.plantype IS '';
ALTER TABLE qgep_od.progression_alternative ADD COLUMN progression_geometry geometry('COMPOUNDCURVE', 2056);
CREATE INDEX in_qgep_progression_alternative_progression_geometry ON qgep_od.progression_alternative USING gist (progression_geometry );
COMMENT ON COLUMN qgep_od.progression_alternative.progression_geometry IS 'Start, inflextion and endpoints of a progression alterative for selected scale (e.g. overview map) / Anfangs-, Knick- und Endpunkte des Alternativverlaufs der Leitung im gewählten Plantyp (z.B. Uebersichtsplan) / Points de départ, intermédiaires et d’arrivée de la trace alternative de la conduite dans la type de plan selectionée';
 ALTER TABLE qgep_od.progression_alternative ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.progression_alternative.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
-------
CREATE TRIGGER
update_last_modified_progression_alternative
BEFORE UPDATE OR INSERT ON
 qgep_od.progression_alternative
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified();
...
COMMENT ON COLUMN qgep_od.organisation.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.organisation ADD COLUMN identifier  varchar(255) ;
COMMENT ON COLUMN qgep_od.organisation.identifier IS 'It is suggested to use real names, e.g. Sample_Community and not only Community. Or "Waste Water Association WWTP Example" and not only Waste Water Association because there will be multiple objects / Es wird empfohlen reale Namen zu nehmen, z.B. Mustergemeinde und nicht Gemeinde. Oder Abwasserverband ARA Muster und nicht nur Abwasserverband, da es sonst Probleme gibt bei der Zusammenführung der Daten. / Utilisez les noms réels, par ex. commune "exemple" et pas seulement commune. Ou "Association pour l''épuration des eaux usées STEP XXX" et pas seulement  Association pour l''épuration des eaux usées. Sinon vous risquer des problèmes en réunissant les données de différentes communes.';
 ALTER TABLE qgep_od.organisation ADD COLUMN identifier_short  varchar(12) ;
COMMENT ON COLUMN qgep_od.organisation.identifier_short IS ' / Kurzbezeichnung / désignation abrégée';
 ALTER TABLE qgep_od.organisation ADD COLUMN municipality_number  smallint ;
COMMENT ON COLUMN qgep_od.organisation.municipality_number IS 'Official number of municipality of federal office for statistics, mandatory for the municipalities. / Offizielle Gemeinde Nummer gemäss Bundesamt für Statistik, bei Gemeinden zwingend, sonst leer lassen. / Numéro officiel de la commune selon l''Office fédéral de la statistique, obligatoire pour les communes. Sinon, laissez vide.';
 ALTER TABLE qgep_od.organisation ADD COLUMN organisation_type  integer ;
COMMENT ON COLUMN qgep_od.organisation.organisation_type IS 'Type of organisatoin / Art der Organisatoin / Genre d''organisation';
 ALTER TABLE qgep_od.organisation ADD COLUMN remark  varchar(255) ;
COMMENT ON COLUMN qgep_od.organisation.remark IS 'General remarks / Allgemeine Bemerkungen / Remarques générales';
 ALTER TABLE qgep_od.organisation ADD COLUMN status  integer ;
COMMENT ON COLUMN qgep_od.organisation.status IS 'yyy_Status der Organisation, damit untergegangende Organisationen nicht einfach gelöscht werden müssen und kontrolliert werden kann, ob noch Beziehungen auf untergegangene Organisationen, z.B. bei Gemeindefusion oder Konkurs einer Firma vorhanden sind / Status der Organisation, damit untergegangende Organisationen nicht einfach gelöscht werden müssen und kontrolliert werden kann, ob noch Beziehungen auf untergegangene Organisationen, z.B. bei Gemeindefusion oder Konkurs einer Firma vorhanden sind / Statut de l''organisation, pour que les organisations disparues ne soient pas simplement supprimées et qu''il soit possible de contrôler s''il existe encore des relations avec les organisations disparues, par exemple en cas de fusion de communes ou de faillite d''une société';
 ALTER TABLE qgep_od.organisation ADD COLUMN uid  varchar(12) ;
COMMENT ON COLUMN qgep_od.organisation.uid IS 'yyy_Referenz zur Unternehmensidentifikation des Bundesamts fuer Statistik (www.uid.admin.ch), e.g. z.B. CHE123456789 / Referenz zur Unternehmensidentifikation des Bundesamts fuer Statistik (www.uid.admin.ch), z.B. CHE123456789 / Référence pour l’identification des entreprises selon l’Office fédéral de la statistique OFS (www.uid.admin.ch), par exemple: CHE123456789';
...
CREATE SEQUENCE qgep_od.seq_measure_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.measure ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','measure');
COMMENT ON COLUMN qgep_od.measure.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.measure ADD COLUMN category  integer ;
COMMENT ON COLUMN qgep_od.measure.category IS 'Category of measure (mandatory) / Massnahmenkategorie (obligatorisch) / Catégorie de la mesure (obligatoire)';
 ALTER TABLE qgep_od.measure ADD COLUMN date_entry  timestamp without time zone ;
COMMENT ON COLUMN qgep_od.measure.date_entry IS 'Entry date, when the measure was added to the list of measures / Datum, an welchem die Massnahme in die Massnahmenliste aufgenommen wurde / Date d''entrée de la mesure dans le plan d''actions';
 ALTER TABLE qgep_od.measure ADD COLUMN description  varchar(100) ;
COMMENT ON COLUMN qgep_od.measure.description IS '';
 ALTER TABLE qgep_od.measure ADD COLUMN identifier  varchar(50) ;
COMMENT ON COLUMN qgep_od.measure.identifier IS 'Identifier of the measure. The identification follows certain rules (see Wegleitung GEP-Daten) / Bezeichnung der Massnahme. Die Bezeichnung erfolgt nach bestimmten Regeln (siehe Wegleitung GEP-Daten) / Désignation de la mesure. La désignation suit des règles précises (cf. guide des données PGEE)';
 ALTER TABLE qgep_od.measure ADD COLUMN intervention_demand  varchar(255) ;
COMMENT ON COLUMN qgep_od.measure.intervention_demand IS 'Short description of need of action / Kurzbeschreibung des Handlungsbedarfs / Description courte du besoin d''intervention';
ALTER TABLE qgep_od.measure ADD COLUMN line_geometry geometry('COMPOUNDCURVE', 2056);
CREATE INDEX in_qgep_measure_line_geometry ON qgep_od.measure USING gist (line_geometry );
COMMENT ON COLUMN qgep_od.measure.line_geometry IS 'yyy_Ermöglicht die Visualisierung einer Massnahme mit einer Linie (optional) / Ermöglicht die Visualisierung einer Massnahme mit einer Linie (optional) / Permet la visualisation d''une mesure à l''aide d''une ligne (optionnelle)';
 ALTER TABLE qgep_od.measure ADD COLUMN link  varchar(255) ;
COMMENT ON COLUMN qgep_od.measure.link IS 'Reference to other measure (identifier)  or works done. Reference to documents, that specify details of the measure, e.g. GEP reports or documents or project papers. / Verweis auf andere Massnahmen (Bezeichnung) oder Arbeiten, Hinweis auf Grundlagen in denen die Massnahmen näher erläutert werden, wie z.B. auf die entsprechenden GEP-Teilprojekte / Référence à d’autres mesures ou travaux, documents explicatifs concernant la mesure, par exemple les projets partiels PGEE ou rapports d’état';
ALTER TABLE qgep_od.measure ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_measure_perimeter_geometry ON qgep_od.measure USING gist (perimeter_geometry );
COMMENT ON COLUMN qgep_od.measure.perimeter_geometry IS 'Perimeter, for visualisation and geometrical relation (OPTIONAL) / Ermöglicht die Visualisierung einer Massnahme mit einer Fläche (optional) / Pour la visualisation et l’illustration avec une surface (optionelle)';
 ALTER TABLE qgep_od.measure ADD COLUMN priority  integer ;
COMMENT ON COLUMN qgep_od.measure.priority IS 'Priority of measure / Priorität der Massnahme / Priorité de la mesure.';
 ALTER TABLE qgep_od.measure ADD COLUMN remark  varchar(80) ;
COMMENT ON COLUMN qgep_od.measure.remark IS 'General remarks of project designer or controlling institution / Bemerkungen des Projektverfassers oder der Aufsichtsbehörde / Remarques du gestionnaire du projet ou de l''autorité de surveillance';
 ALTER TABLE qgep_od.measure ADD COLUMN status  integer ;
COMMENT ON COLUMN qgep_od.measure.status IS 'Disposition state of measure / Status der Massnahme / Etat de la mesure';
ALTER TABLE qgep_od.measure ADD COLUMN symbolpos_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_measure_symbolpos_geometry ON qgep_od.measure USING gist (symbolpos_geometry );
COMMENT ON COLUMN qgep_od.measure.symbolpos_geometry IS 'For the visualisation (without geometric relation) / Für die Visualisierung (ohne räumlichen Bezug) / Pour la visualisation (sans relation géométrique)';
 ALTER TABLE qgep_od.measure ADD COLUMN total_cost  decimal(10,2) ;
COMMENT ON COLUMN qgep_od.measure.total_cost IS 'Sum of own and cost of third parties. Eventually they can be listed also seperately. / Summe der Eigenleistung und Kosten Dritter. Allenfalls können diese zusätzlich auch separat ausgewiesen werden / Somme des contributions propres et des coûts de parties tiers. Ils peuvent également être justifiés séparément';
 ALTER TABLE qgep_od.measure ADD COLUMN year_implementation_effectiv  smallint ;
COMMENT ON COLUMN qgep_od.measure.year_implementation_effectiv IS 'Year the measure was actually implemented / Jahr, in dem die Massnahme effektiv umgesetzt wurde / Année à laquelle la mesure a effectivement été mise en œuvre';
 ALTER TABLE qgep_od.measure ADD COLUMN year_implementation_planned  smallint ;
COMMENT ON COLUMN qgep_od.measure.year_implementation_planned IS 'Planned year of implementation / Jahr bis die Massnahme umgesetzt sein soll / Année à laquelle la mesure devrait être mise en œuvre';
 ALTER TABLE qgep_od.measure ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.measure.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.measure ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.measure.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.measure ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.measure.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
...
COMMENT ON COLUMN qgep_od.mutation.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.mutation ADD COLUMN attribute  varchar(60) ;
...
COMMENT ON COLUMN qgep_od.mutation.kind IS '';
 ALTER TABLE qgep_od.mutation ADD COLUMN last_value  varchar(255) ;
COMMENT ON COLUMN qgep_od.mutation.last_value IS 'last_value changed to text. Only with type=changed and deleted / Letzter Wert umgewandelt in Text. Nur bei ART=geaendert oder geloescht / Dernière valeur modifiée du texte. Seulement avec GENRE = changee ou effacee';
 ALTER TABLE qgep_od.mutation ADD COLUMN object  varchar(16) ;
COMMENT ON COLUMN qgep_od.mutation.object IS 'OID of Object / OID des Objektes / OID de l''objet';
 ALTER TABLE qgep_od.mutation ADD COLUMN recorded_by  varchar(255) ;
...
COMMENT ON COLUMN qgep_od.mutation.remark IS 'General remarks / Allgemeine Bemerkungen / Remarques générales';
 ALTER TABLE qgep_od.mutation ADD COLUMN system_user  varchar(60) ;
...
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.elimination_p IS 'Dimensioning value elimination rate in percent / Dimensionierungswert Eliminationsrate in % / Valeur de dimensionnement, taux d''élimination en %';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN identifier  varchar(255) ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.identifier IS 'yyy_Bezeichnung der Abwasserreinigungsanlage gemäss Liste BAFU / Bezeichnung der Abwasserreinigungsanlage gemäss Liste BAFU / Désignation de la STEP selon la liste de l''Office fédéral de l''environnement (OFEV)';
...
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.nh4 IS 'yyy_Dimensioning value Ablauf Vorklärung. NH4 [gNH4/m3] / Dimensionierungswert Ablauf Vorklärung. NH4 [gNH4/m3] / Valeur de dimensionnement, NH4 à la sortie du décanteur primaire. NH4 [gNH4/m3]';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN operator_type  integer ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.operator_type IS 'yyy_Organisationsform der betreibenden Organisation / Organisationsform der betreibenden Organisation / Forme organisationnelle de l’exploitant';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN population_connected  integer ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.population_connected IS ' / VSA-Kennzahl "Anzahl angeschlossene Einwohner" [E], Matching MGDM 134.5 EinwAngeschlossen / Indicateur du VSA « Nombre d’habitants raccordés » [H], Matching MGDM 134.5 EinwAngeschlossen';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN population_total  integer ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.population_total IS ' / VSA-Kennzahl "Anzahl Einwohner Total" [E], Matching MGDM 134.5 EinwAnz / Indicateur du VSA « Nombre total d’habitants » [H], Matching MGDM 134.5 EinwAnz';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN remark  varchar(255) ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.remark IS 'General remarks / Allgemeine Bemerkungen / Remarques générales';
ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_waste_water_treatment_plant_situation_geometry ON qgep_od.waste_water_treatment_plant USING gist (situation_geometry );
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.situation_geometry IS 'yyy_Standort der ARA, Mitte des Geländes / Standort der ARA, Mitte des Geländes / Site de la STEP, milieu du site';
...
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.start_year IS 'Start of operation (year) / Jahr der Inbetriebnahme / Année de la mise en exploitation';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN wwtp_number  integer ;
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.wwtp_number IS 'WWTP Number from Federal Office for the Environment (FOEN) / ARA-Nummer gemäss Bundesamt für Umwelt (BAFU) / Numéro de la STEP selon l''Office fédéral de l''environnement (OFEV)';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.waste_water_treatment_plant.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
...
COMMENT ON COLUMN qgep_od.wastewater_structure.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.wastewater_structure ADD COLUMN detail_geometry_geometry geometry('CURVEPOLYGONZ', 2056);
CREATE INDEX in_qgep_wastewater_structure_detail_geometry_geometry ON qgep_od.wastewater_structure USING gist (detail_geometry_geometry );
...
COMMENT ON COLUMN qgep_od.wastewater_structure.gross_costs IS 'Gross costs of construction / Brutto Erstellungskosten / Coûts bruts des travaux de construction';
 ALTER TABLE qgep_od.wastewater_structure ADD COLUMN identifier  varchar(41) ;
COMMENT ON COLUMN qgep_od.wastewater_structure.identifier IS 'Unique designation per data owner / Pro Datenherr eindeutige Bezeichnung / Désignation unique pour chaque maître des données';
...
COMMENT ON COLUMN qgep_od.channel.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.channel.connection_type IS 'Types of connection / Verbindungstypen / Types de raccordement';
 ALTER TABLE qgep_od.channel ADD COLUMN function_amelioration  integer ;
COMMENT ON COLUMN qgep_od.channel.function_amelioration IS 'yyy_Zur Unterscheidung der Funktion einer Leitung bei Meliorationen (Entwässerungen) / Zur Unterscheidung der Funktion einer Leitung bei Meliorationen (Entwässerungen) / Afin de distinguer la fonction d’une conduite d’amélioration foncière (drainage)';
 ALTER TABLE qgep_od.channel ADD COLUMN function_hierarchic  integer ;
COMMENT ON COLUMN qgep_od.channel.function_hierarchic IS 'yyy_Art des Kanals hinsichtlich der hierarchischen Bedeutung im Entwässerungssystem / Art des Kanals hinsichtlich der hierarchischen  Bedeutung im Entwässerungssystem / Genre de canalisation par rapport à sa fonction hiérarchique dans le système d''évacuation';
-- see end of table CREATE INDEX in_channel_function_hierarchic_usage_current ON qgep_od.channel USING btree (function_hierarchic, usage_current);
...
COMMENT ON COLUMN qgep_od.channel.pipe_length IS 'yyy_Baulänge der Einzelrohre oder Fugenabstände bei Ortsbetonkanälen / Baulänge der Einzelrohre oder Fugenabstände bei Ortsbetonkanälen / Longueur de chaque tuyau ou distance des joints pour les canalisations en béton coulé sur place';
 ALTER TABLE qgep_od.channel ADD COLUMN seepage  integer ;
COMMENT ON COLUMN qgep_od.channel.seepage IS 'yyy Beschreibung des oberliegenden Materials bei Saugern / Beschreibung des oberliegenden Materials bei Saugern / Description du matériau de remplissage';
...
COMMENT ON COLUMN qgep_od.channel.usage_planned IS 'yyy_Durch das Konzept vorgesehene Nutzung (vergleiche auch Nutzungsart_Ist) / Durch das Konzept vorgesehene Nutzung (vergleiche auch Nutzungsart_Ist) / Utilisation prévue par le concept d''assainissement (voir aussi GENRE_UTILISATION_ACTUELLE)';
 CREATE INDEX in_channel_function_hierarchic_usage_current ON qgep_od.channel USING btree (function_hierarchic, usage_current);
-------
CREATE TRIGGER
...
COMMENT ON COLUMN qgep_od.manhole.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.manhole ADD COLUMN amphibian_exit  integer ;
COMMENT ON COLUMN qgep_od.manhole.amphibian_exit IS 'Structural measures for the exit of amphibians available. / Bauliche Massnahme für den Ausstieg von Amphibien vorhanden. / Des mesures structurelles pour la sortie des amphibiens sont en place.';
...
COMMENT ON COLUMN qgep_od.manhole.function IS 'Kind of function / Art der Nutzung / Genre d''utilisation';
 CREATE INDEX in_manhole_function ON qgep_od.manhole USING btree (function);
...
COMMENT ON COLUMN qgep_od.manhole.material IS 'yyy_Hauptmaterial aus dem das Bauwerk besteht zur groben Klassifizierung. / Hauptmaterial aus dem das Bauwerk besteht zur groben Klassifizierung. / Matériau dont est construit l''ouvrage, pour une classification sommaire';
 ALTER TABLE qgep_od.manhole ADD COLUMN possibility_intervention  integer ;
COMMENT ON COLUMN qgep_od.manhole.possibility_intervention IS 'Intervention possibility on the wastewater structure for the fire department available. / Interventionsmöglichkeit auf dem Bauwerk für die Wehrdienste vorhanden. / Possibilité d''intervention sur l''ouvrage pour les services du feu (pompiers).';
...
COMMENT ON COLUMN qgep_od.discharge_point.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.discharge_point.relevance IS 'Relevance of discharge point for water course / Gewässerrelevanz der Einleitstelle / Signifiance pour milieu récepteur';
...
COMMENT ON COLUMN qgep_od.discharge_point.upper_elevation IS 'Highest point of structure (ceiling), outside / Höchster Punkt des Bauwerks (Decke), aussen / Point le plus élevé de l''ouvrage';
 ALTER TABLE qgep_od.discharge_point ADD COLUMN water_course_number  varchar(25) ;
COMMENT ON COLUMN qgep_od.discharge_point.water_course_number IS 'Watercourse number according to VECTOR25 watercourse network GWN07 (Federal Office of Topography swisstopo) / Gewässerlaufnummer gemäss VECTOR25 Gewässernetz GWN07 (Bundesamt für Landestopografie swisstopo) / Numéro du cours d’eau selon VECTOR25 Réseau hydrographique GWN07 (Office fédéral de topographie swisstopo)';
 ALTER TABLE qgep_od.discharge_point ADD COLUMN water_course_segment_canton  varchar(36) ;
COMMENT ON COLUMN qgep_od.discharge_point.water_course_segment_canton IS 'yyy_Designation of the section in the cantonal watercourse network / Bezeichnung des Gewässerabschnittes im Kantonalen Gewässernetz / Désignation du tronçon dans le réseau cantonal des cours d''eau';
...
COMMENT ON COLUMN qgep_od.special_structure.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.special_structure ADD COLUMN amphibian_exit  integer ;
COMMENT ON COLUMN qgep_od.special_structure.amphibian_exit IS 'Structural measures for the exit of amphibians available. / Bauliche Massnahme für den Ausstieg von Amphibien vorhanden. / Des mesures structurelles pour la sortie des amphibiens sont en place.';
...
COMMENT ON COLUMN qgep_od.special_structure.function IS 'Kind of function / Art der Nutzung / Genre d''utilisation';
 CREATE INDEX in_special_structure_function ON qgep_od.special_structure USING btree (function);
 ALTER TABLE qgep_od.special_structure ADD COLUMN possibility_intervention  integer ;
COMMENT ON COLUMN qgep_od.special_structure.possibility_intervention IS 'Intervention possibility on the wastewater structure for the fire department available. / Interventionsmöglichkeit auf dem Bauwerk für die Wehrdienste vorhanden. / Possibilité d''intervention sur l''ouvrage pour les services du feu (pompiers).';
...
COMMENT ON COLUMN qgep_od.infiltration_installation.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.infiltration_installation.seepage_utilization IS 'Types of water to be infiltrated. / Arten des zu versickernden Wassers. / Genre d''eau à infiltrer';
...
COMMENT ON COLUMN qgep_od.wwtp_structure.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.maintenance_event.result IS 'Result or important comments for this event from the perspective of the editor / Resultat oder wichtige Bemerkungen aus Sicht des Bearbeiters / Résultat ou commentaire importante de l''événement du point de vue de l''éditeur';
...
COMMENT ON COLUMN qgep_od.zone.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.infiltration_zone.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.infiltration_zone ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_infiltration_zone_perimeter_geometry ON qgep_od.infiltration_zone USING gist (perimeter_geometry );
...
COMMENT ON COLUMN qgep_od.drainage_system.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.drainage_system ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_drainage_system_perimeter_geometry ON qgep_od.drainage_system USING gist (perimeter_geometry );
...
COMMENT ON COLUMN qgep_od.pipe_profile.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.wwtp_energy_use.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.waste_water_treatment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.sludge_treatment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.control_center.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.control_center ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_control_center_situation_geometry ON qgep_od.control_center USING gist (situation_geometry );
...
COMMENT ON COLUMN qgep_od.hydr_geometry.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.reach_point.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.reach_point.outlet_shape IS 'Kind of outlet shape / Art des Auslaufs / Types de sortie';
 ALTER TABLE qgep_od.reach_point ADD COLUMN pipe_closure  integer ;
COMMENT ON COLUMN qgep_od.reach_point.pipe_closure IS 'yyy_Rohrverschluss oder -kappe am Anfang oder Ende der Leitung. Default Wert ist nein. / Rohrverschluss oder -kappe am Anfang oder Ende der Leitung. Default Wert ist nein. / Tuyau bouchonné au début ou fin d''une conduite. La valeur par défaut est non.';
...
ALTER TABLE qgep_od.reach_point ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_reach_point_situation_geometry ON qgep_od.reach_point USING gist (situation_geometry );
...
ALTER TABLE qgep_od.wastewater_node ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_wastewater_node_situation_geometry ON qgep_od.wastewater_node USING gist (situation_geometry );
COMMENT ON COLUMN qgep_od.wastewater_node.situation_geometry IS 'yyy Situation of node. Decisive reference point for sewer network simulation  (In der Regel Lage des Pickellochs oder Lage des Trockenwetterauslauf) / Lage des Knotens, massgebender Bezugspunkt für die Kanalnetzberechnung. (In der Regel Lage des Pickellochs oder Lage des Trockenwetterauslaufs) / Positionnement du nœud. Point de référence déterminant pour le calcul de réseau de canalisations (en règle générale positionnement du milieu du couvercle ou de la sortie temps sec)';
 ALTER TABLE qgep_od.wastewater_node ADD COLUMN wwtp_number  integer ;
...
COMMENT ON COLUMN qgep_od.reach.elevation_determination IS 'yyy_Definiert die Hoehenbestimmung einer Haltung. / Definiert die Hoehenbestimmung einer Haltung. / Définition de la détermination altimétrique d''un tronçon.';
 ALTER TABLE qgep_od.reach ADD COLUMN flow_time_dry_weather  decimal(7,0) ;
COMMENT ON COLUMN qgep_od.reach.flow_time_dry_weather IS 'Flow time in dry weather (daily mean) / Fliesszeit bei Trockenwetter (Tagesmittel) / Temps d''écoulement par temps sec (moyenne journalière)';
...
COMMENT ON COLUMN qgep_od.reach.horizontal_positioning IS 'yyy_Definiert die Lagegenauigkeit der Verlaufspunkte. / Definiert die Lagegenauigkeit der Verlaufspunkte. / Définit la précision de la détermination du tracé.';
 ALTER TABLE qgep_od.reach ADD COLUMN hydraulic_load_current  smallint ;
COMMENT ON COLUMN qgep_od.reach.hydraulic_load_current IS 'Dimensioning of the discharge divided by the normal discharge capacity of the reach [%]. / Dimensionierungsabfluss geteilt durch Normalabflusskapazität der Leitung [%]. / Débit de dimensionnement divisé par la capacité d''écoulement normale de la conduite [%].';
...
ALTER TABLE qgep_od.reach ADD COLUMN progression_geometry geometry('COMPOUNDCURVE', 2056);
CREATE INDEX in_qgep_reach_progression_geometry ON qgep_od.reach USING gist (progression_geometry );
...
ALTER TABLE qgep_od.reach ADD COLUMN progression3d_geometry geometry('COMPOUNDCURVEZ', 2056);
CREATE INDEX in_qgep_reach_progression3d_geometry ON qgep_od.reach USING gist (progression3d_geometry );
COMMENT ON COLUMN qgep_od.reach.progression3d_geometry IS 'Start, inflextion and endpoints of a pipe (3D coordinates) / Anfangs-, Knick- und Endpunkte der Leitung (3D Koordinaten) / Points de départ, intermédiaires et d’arrivée de la conduite (coordonnées 3D)';
...
COMMENT ON COLUMN qgep_od.reach.reliner_nominal_size IS 'Clear height with installed liner (= clear height of the original pipe profile minus twice the wall thickness of the liner). / Lichte Höhe mit eingebautem Liner (=Lichte Höhe des ursprünglichen Rohrprofils minus doppelte Wandstärke des Liners). / Hauteur libre avec revêtement installé (= hauteur libre du profilé d''origine du tuyau moins deux fois l''épaisseur de la paroi du revêtement).';
...
COMMENT ON COLUMN qgep_od.profile_geometry.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.profile_geometry ADD COLUMN sequence  smallint ;
COMMENT ON COLUMN qgep_od.profile_geometry.sequence IS 'Sequence of the detail points of geometry definition / Reihenfolge der Detailpunkte der Geometriedefinition / Ordre des points de détail de la définition de la géométrie';
 ALTER TABLE qgep_od.profile_geometry ADD COLUMN x  real ;
COMMENT ON COLUMN qgep_od.profile_geometry.x IS 'X-coordinate / X-Koordinate / l''abscisse (X)';
 ALTER TABLE qgep_od.profile_geometry ADD COLUMN y  real ;
COMMENT ON COLUMN qgep_od.profile_geometry.y IS 'Y-coordinate / Y-Koordinate / l''ordonnée (Y)';
...
COMMENT ON COLUMN qgep_od.hydr_geom_relation.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.mechanical_pretreatment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.retention_body.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.overflow_char.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.hq_relation.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.structure_part.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.dryweather_downspout.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.dryweather_downspout ADD COLUMN diameter  smallint ;
COMMENT ON COLUMN qgep_od.dryweather_downspout.diameter IS '';
-------
CREATE TRIGGER
...
COMMENT ON COLUMN qgep_od.access_aid.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.dryweather_flume.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.cover.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.cover ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_cover_situation_geometry ON qgep_od.cover USING gist (situation_geometry );
...
COMMENT ON COLUMN qgep_od.electric_equipment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.electric_equipment ADD COLUMN gross_costs  decimal(10,2) ;
COMMENT ON COLUMN qgep_od.electric_equipment.gross_costs IS 'Gross costs of electrical equipment / Brutto Erstellungskosten der elektrischen Ausrüstung / Coûts bruts des équipements électriques';
...
COMMENT ON COLUMN qgep_od.electromechanical_equipment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.benching.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
-------
-------
CREATE TABLE qgep_od.flushing_nozzle
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_flushing_nozzle_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_flushing_nozzle_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.flushing_nozzle ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','flushing_nozzle');
COMMENT ON COLUMN qgep_od.flushing_nozzle.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
ALTER TABLE qgep_od.flushing_nozzle ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_flushing_nozzle_situation_geometry ON qgep_od.flushing_nozzle USING gist (situation_geometry );
COMMENT ON COLUMN qgep_od.flushing_nozzle.situation_geometry IS '';
-------
CREATE TRIGGER
update_last_modified_flushing_nozzle
BEFORE UPDATE OR INSERT ON
 qgep_od.flushing_nozzle
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified_parent("qgep_od.structure_part");
...
COMMENT ON COLUMN qgep_od.connection_object.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.building.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.building ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_building_perimeter_geometry ON qgep_od.building USING gist (perimeter_geometry );
...
ALTER TABLE qgep_od.building ADD COLUMN reference_point_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_building_reference_point_geometry ON qgep_od.building USING gist (reference_point_geometry );
...
COMMENT ON COLUMN qgep_od.reservoir.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.reservoir ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_reservoir_situation_geometry ON qgep_od.reservoir USING gist (situation_geometry );
...
COMMENT ON COLUMN qgep_od.individual_surface.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.individual_surface ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_individual_surface_perimeter_geometry ON qgep_od.individual_surface USING gist (perimeter_geometry );
...
COMMENT ON COLUMN qgep_od.fountain.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.fountain ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_fountain_situation_geometry ON qgep_od.fountain USING gist (situation_geometry );
...
CREATE SEQUENCE qgep_od.seq_log_card_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.log_card ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','log_card');
COMMENT ON COLUMN qgep_od.log_card.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.log_card ADD COLUMN control_remote_control  integer ;
COMMENT ON COLUMN qgep_od.log_card.control_remote_control IS 'In contrast to the Control attribute in the VSA-DSS classes overflow and throttle_shut_off_unit, the present attribute refers to the entire control/alarm of the special structure. The value range is meant to be cumulative in the following order: no_control, local_control, transmission_alarm, transmission_measuring_signals, interconnection. / Im Gegensatz zum Attribut Steuerung in den VSA-DSS-Klassen Ueberlauf und Absperr_Drosselorgan bezieht sich das vorliegende Attribut auf die gesamte Steuerung/Alarmierung des Sonderbauwerks. / Contrairement à l''attribut COMMANDE dans les classes VSA-SDEE DEVERSOIR et LIMITEUR_DEBIT, cet attribut fait référence à l''ensemble commande/alarme de l''ouvrage spécial.';
 ALTER TABLE qgep_od.log_card ADD COLUMN information_source  integer ;
COMMENT ON COLUMN qgep_od.log_card.information_source IS 'Categories for information sources / Für die Quellen stehen die angegebenen Möglichkeiten zur Verfügung. / Valeurs à disposition pour les sources d’information sur une liste à choix';
 ALTER TABLE qgep_od.log_card ADD COLUMN person_in_charge  varchar(50) ;
COMMENT ON COLUMN qgep_od.log_card.person_in_charge IS 'Person in charge that created the log_card / Sachbearbeiter, der die Stammkarte erstellt hat. / Technicien ayant remplir la fiche technique.';
 ALTER TABLE qgep_od.log_card ADD COLUMN remark  varchar(80) ;
COMMENT ON COLUMN qgep_od.log_card.remark IS 'General remarks';
 ALTER TABLE qgep_od.log_card ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.log_card.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.log_card ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.log_card.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.log_card ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.log_card.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
...
COMMENT ON COLUMN qgep_od.catchment_area.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.catchment_area.direct_discharge_planned IS 'The rain water will be discharged fully or partially over a SAA pipe into a water body / Das Niederschlagsabwasser wird in Zukunft ganz oder teilweise über eine SAA-Leitung in ein Gewässer eingeleitet / Les eaux pluviales seront rejetées complètement ou partiellement via une conduite OAS dans un cours d’eau';
 ALTER TABLE qgep_od.catchment_area ADD COLUMN discharge_coefficient_rw_current  decimal (5,2) ;
COMMENT ON COLUMN qgep_od.catchment_area.discharge_coefficient_rw_current IS 'yyy_Abflussbeiwert für den Niederschlagsabwasseranschluss im Ist-Zustand / Abflussbeiwert für den Niederschlagsabwasseranschluss im Ist-Zustand / Coefficient de ruissellement pour le raccordement actuel des eaux pluviales';
 ALTER TABLE qgep_od.catchment_area ADD COLUMN discharge_coefficient_rw_planned  decimal (5,2) ;
COMMENT ON COLUMN qgep_od.catchment_area.discharge_coefficient_rw_planned IS 'yyy_Abflussbeiwert für den Niederschlagsabwasseranschluss im Planungszustand / Abflussbeiwert für den Niederschlagsabwasseranschluss im Planungszustand / Coefficient de ruissellement prévu pour le raccordement des eaux pluviales';
...
COMMENT ON COLUMN qgep_od.catchment_area.infiltration_current IS 'yyy_Das Niederschlagsabwasser wird ganz oder teilweise einer Versickerungsanlage zugeführt / Das Niederschlagsabwasser wird ganz oder teilweise einer Versickerungsanlage zugeführt / Les eaux pluviales sont amenées complètement ou partiellement à une installation d’infiltration';
...
ALTER TABLE qgep_od.catchment_area ADD COLUMN perimeter_geometry geometry('CURVEPOLYGON', 2056);
CREATE INDEX in_qgep_catchment_area_perimeter_geometry ON qgep_od.catchment_area USING gist (perimeter_geometry );
...
COMMENT ON COLUMN qgep_od.catchment_area.seal_factor_rw_current IS 'yyy_Befestigungsgrad für den Niederschlagsabwasseranschluss im Ist-Zustand / Befestigungsgrad für den Niederschlagsabwasseranschluss im Ist-Zustand / Taux d''imperméabilisation pour le raccordement eaux pluviales actuel';
 ALTER TABLE qgep_od.catchment_area ADD COLUMN seal_factor_rw_planned  decimal (5,2) ;
COMMENT ON COLUMN qgep_od.catchment_area.seal_factor_rw_planned IS 'yyy_Befestigungsgrad für den Niederschlagsabwasseranschluss im Planungszustand / Befestigungsgrad für den Niederschlagsabwasseranschluss im Planungszustand / Taux d''imperméabilisation pour le raccordement eaux pluviales prévu';
...
COMMENT ON COLUMN qgep_od.surface_runoff_parameters.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.measuring_point.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
ALTER TABLE qgep_od.measuring_point ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_measuring_point_situation_geometry ON qgep_od.measuring_point USING gist (situation_geometry );
...
COMMENT ON COLUMN qgep_od.measuring_device.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.measurement_series.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.measurement_result.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.overflow.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.throttle_shut_off_unit.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.prank_weir.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.pump.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.pump.construction_type IS 'Types of pumps / Pumpenarten / Types de pompe';
 ALTER TABLE qgep_od.pump ADD COLUMN operating_point  decimal(8,3) ;
...
COMMENT ON COLUMN qgep_od.leapingweir.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
-------
-------
CREATE TABLE qgep_od.maintenance
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_maintenance_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_maintenance_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.maintenance ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','maintenance');
COMMENT ON COLUMN qgep_od.maintenance.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
-------
CREATE TRIGGER
...
-------
-------
CREATE TABLE qgep_od.bio_ecol_assessment
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_bio_ecol_assessment_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_bio_ecol_assessment_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.bio_ecol_assessment ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','bio_ecol_assessment');
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN comparison_last  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.comparison_last IS 'yyy_Die Veränderung der Gesamtbeurteilung und eventuelle massgebende veränderte Untersuchungs-resultate gegenüber der letzten Untersuchung müssen dokumentiert werden. / Die Veränderung der Gesamtbeurteilung und eventuelle massgebende veränderte Untersuchungsresultate gegenüber der letzten Untersuchung müssen dokumentiert werden. / Les variations de l’examen générale éco-biologique et des résultats déterminants ayant changés doivent être indiquées par rapport à la dernière inspection.';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN date_last_examen  timestamp without time zone ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.date_last_examen IS 'yyy_Datum der letzten Untersuchung, falls vorhanden. Das Datum der aktuellen Untersuchung wird im Attribut Datum_Untersuchung (VSA-DSS-Mini) bzw. Zeitpunkt (VSA-DSS) erfasst. / Datum der letzten Untersuchung, falls vorhanden. Das Datum der aktuellen Untersuchung wird im Attribut Datum_Untersuchung (VSA-DSS-Mini) bzw. Zeitpunkt (VSA-DSS) erfasst. / Date de l''examen précedent. La date de l’examen actuel est saisie dans l''attribut DATE_EXAMEN (VSA-SDEE-MINI) ou DATE_HEURE (VSA-DSS)';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN impact_auxiliary_indic  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.impact_auxiliary_indic IS 'yyy_Nur für stehende Gewässer / Nur für stehende Gewässer / Uniquement pour eaux stagnantes';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN impact_external_aspect  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.impact_external_aspect IS 'yyy_Resultiert aus dem Vergleich des äusseren Aspekts unterhalb und oberhalb der Einleitstelle / Resultiert aus dem Vergleich des äusseren Aspekts unterhalb und oberhalb der Einleitstelle / Résultat de la comparaison de l''aspect visuel en aval et en amont de l''exutoire';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN impact_macroinvertebrates  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.impact_macroinvertebrates IS 'yyy_Nur für Fliessgewässer. Resultiert aus dem Vergleich der Makroinvertebraten unterhalb und oberhalb der Einleitstelle gemäss dem Modul Gewässeruntersuchung der VSA-Richtlinie Abwasserbewirtschaftung bei Regenwetter. Deshalb nicht zu verwechseln mit den aufgehobenen Attributen Makroinvertebraten_oberhalb, _Einleitung und _unterhalb. / Nur für Fliessgewässer. Resultiert aus dem Vergleich der Makroinvertebraten unterhalb und oberhalb der Einleitstelle gemäss dem Modul Gewässeruntersuchung der VSA-Richtlinie Abwasserbewirtschaftung bei Regenwetter. / Uniquement pour des eaux courantes. Résultat de la comparaison des invertébrés aquatiques en aval et en amont de l''exutoire selon le module Analyse de la qualité des eaux de la directive Gestion des eaux pluviales du VSA. Ne pas confondre avec les attributs INVERTEBREES_EN_AMONT, INVERTEBREES_EXUTOIRE et INVERTEBREES_EN_AVAL.';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN impact_water_plants  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.impact_water_plants IS 'yyy_Nur für stehende Gewässer / Nur für stehende Gewässer / Uniquement pour eaux stagnantes';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN intervention_demand  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.intervention_demand IS 'Need for action resulting from the impairment of the discharge point on the water body, which leads to a measure in the action plan. The attribute is also used to derive the "total impairment" in the MGDM 129.1 of the FOEN, as long as it is still kept there. / Handlungsbedarf resultierend aus der Beeinträchtigung der Einleitstelle auf das Gewässer, der zu einer Massnahme im Massnahmenplan führt. Das Attribut dient auch zur Ableitung der "Gesamtbeeintraechtigung" im MGDM 129.1 des BAFU, solange dieses dort noch geführt wird. / Un besoin d’intervention résulte de l’atteinte du rejet sur les eaux qui mène à une mesure dans le plan d’action du PGEE. L''attribut sert aussi à la détermination de "l''attteinte globale" dans le MGDM 129.1 de l''OFEV si celui-ci est encore géré.';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN io_calculation  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.io_calculation IS 'immission oriented calculation available / Immissionsorientierte Berechnung vorhanden. / calcul de performance de type immission disponible';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN outlet_pipe_clear_height  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.outlet_pipe_clear_height IS 'Maximum internal height of the outlet. Helps to identify the correct discharge point in the field. / Maximale Innenhöhe des Auslaufes. Hilft bei der Identifikation der richtigen Einleitstelle im Feld. / Hauteur intérieure maximale de la sortie. Utile pour identifier le bon exutoire sur le terrain.';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN q347  decimal(8,3) ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.q347 IS 'yyy_Menge aus hydrologischen Jahrbüchern. Fehlt diese Angabe in den Jahrbüchern, ist eine Menge zu bestimmen. / Menge aus hydrologischen Jahrbüchern. Fehlt diese Angabe in den Jahrbüchern, ist eine Menge zu bestimmen. / Valeur issue des annuaires hydrologique de Suisse. Si elle manque, il faut désigner une valeur.';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN relevance_matrix  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.relevance_matrix IS 'yyy_Relevanzmatrix gemäss den Vorgaben in der Richtlinie "Abwasserentsorgung bei Regenwetter", Modul STORM des VSA (2019) / Relevanzmatrix gemäss den Vorgaben in der Richtlinie "Abwasserentsorgung bei Regenwetter", Modul STORM des VSA (2019) / Matrice d’évaluation selon les indications de la directive "Gestion des eaux urbaines par temps de pluie", module STORM du VSA (2019)';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN relevant_slope  smallint ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.relevant_slope IS 'zzz_Relevantes Gefälle [%] bei der Einleitstelle (für STORM Berechnung). Falls unbekannt muss das Gefälle im Feld oder aufgrund von Plangrundlagen bestimmt werden. / Relevantes Gefälle [%] bei der Einleitstelle (für STORM Berechnung). Falls unbekannt muss das Gefälle im Feld oder aufgrund von Plangrundlagen bestimmt werden / Pente déterminante [%] de l’exutoire (pour le calcul STORM). Si inconnue, la pente doit être déterminée sur la base de plans ou par une visite de terrain';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN surface_water_bodies  varchar(100) ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.surface_water_bodies IS 'Name of surface water body corresponding to cantonal rules / Gewässername gemäss kantonalen Vorgaben / Nom du cours d’eau selon indications cantonales';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN type_water_body  integer ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.type_water_body IS 'based on table 5.1 of STORM module in directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019/2021) / gemäss Tabelle 5.1 des Moduls STORM der Richtlinie "Abwasserbewirtschaftung bei Regenwetter" des VSA (2019/2021) / selon table 5.1 du module STORM de la directive "Gestion des eaux urbaines par temps de pluie" (2019/2021)';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN water_specific_discharge_freight_nh4_n_current  smallint ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.water_specific_discharge_freight_nh4_n_current IS 'based on base module chapter 8.4. of directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019)" / gemäss Basismodul Kapitel 8.4 der Richtlinie "Abwasserbewirtschaftung bei Regenwetter" des VSA (2019) / Selon module de base chapitre 8.4 de la directive "Gestion des eaux urbaines par temps de pluie" du VSA (2019)';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN water_specific_discharge_freight_nh4_n_current_opt  smallint ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.water_specific_discharge_freight_nh4_n_current_opt IS 'based on base module chapter 8.4. of directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019)" / gemäss Basismodul Kapitel 8.4 der Richtlinie "Abwasserbewirtschaftung bei Regenwetter" des VSA (2019) / Selon module de base chapitre 8.4 de la directive "Gestion des eaux urbaines par temps de pluie" du VSA (2019)';
 ALTER TABLE qgep_od.bio_ecol_assessment ADD COLUMN water_specific_discharge_freight_nh4_n_planned  smallint ;
COMMENT ON COLUMN qgep_od.bio_ecol_assessment.water_specific_discharge_freight_nh4_n_planned IS 'based on base module chapter 8.4. of directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019)" / gemäss Basismodul Kapitel 8.4 der Richtlinie "Abwasserbewirtschaftung bei Regenwetter" des VSA (2019) / Selon module de base chapitre 8.4 de la directive "Gestion des eaux urbaines par temps de pluie" du VSA (2019)';
-------
CREATE TRIGGER
update_last_modified_bio_ecol_assessment
BEFORE UPDATE OR INSERT ON
 qgep_od.bio_ecol_assessment
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified_parent("qgep_od.maintenance_event");
...
COMMENT ON COLUMN qgep_od.hydraulic_char_data.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.hydraulic_char_data ADD COLUMN aggregate_number  smallint ;
COMMENT ON COLUMN qgep_od.hydraulic_char_data.aggregate_number IS 'Number of aggregates / Anzahl Förderaggregate / Nombre d''installations de refoulement';
...
COMMENT ON COLUMN qgep_od.backflow_prevention.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.solids_retention.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.solids_retention.gross_costs IS 'Gross costs of electromechanical equipment / Brutto Erstellungskosten der elektromechanischen Ausrüstung für den Feststoffrueckhalt / Coûts bruts des équipements électromécaniques';
...
COMMENT ON COLUMN qgep_od.tank_cleaning.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.tank_cleaning ADD COLUMN gross_costs  decimal(10,2) ;
COMMENT ON COLUMN qgep_od.tank_cleaning.gross_costs IS 'Gross costs of electromechanical equipment of tank cleaning / Brutto Erstellungskosten der elektromechanischen Ausrüstung für die Beckenreinigung / Coûts bruts des équipements électromécaniques nettoyage de bassins';
...
COMMENT ON COLUMN qgep_od.tank_emptying.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.tank_emptying.gross_costs IS 'Gross costs of electromechanical equipment of tank emptying / Brutto Erstellungskosten der elektromechanischen Ausrüstung für die Beckenentleerung / Coûts bruts des équipements électromécaniques vidange de bassins';
...
-------
-------
CREATE TABLE qgep_od.catchement_area_totals
(
   obj_id varchar(16) NOT NULL,
...
CREATE SEQUENCE qgep_od.seq_catchement_area_totals_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.catchement_area_totals ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','catchement_area_totals');
COMMENT ON COLUMN qgep_od.catchement_area_totals.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN discharge_freight_nh4_n  integer ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.discharge_freight_nh4_n IS 'based on base module chapter 8.5. of directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019)" / Gemäss Basismodul Kapitel 8.5 der Richtlinie "Abwasserentsorgung bei Regenwetter" des VSA (2019) / Selon module de base chapitre 8.5 directive "Gestion des eaux urbaines par temps de pluie" du VSA (2019)';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN discharge_proportion_nh4_n  decimal (5,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.discharge_proportion_nh4_n IS 'based on base module chapter 8.5. of directive "Abwasserbewirtschaftung bei Regenwetter" of VSA (2019)" / Gemäss Basismodul Kapitel 8.5 der Richtlinie "Abwasserentsorgung bei Regenwetter" des VSA (2019) / Selon module de base chapitre 8.5 directive "Gestion des eaux urbaines par temps de pluie" du VSA (2019)';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN identifier  varchar(20) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.identifier IS '';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN population  integer ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.population IS 'Number of inhabitants (population) in the direct catchement area as informative value. Use  im direkten Einzugsgebiet als informativer Wert. Der massgebende Schmutzabwasseranfall ist im gleichnamigen entsprechenden Attribut anzugeben. / Anzahl Einwohner im direkten Einzugsgebiet als informativer Wert. Der massgebende Schmutzabwasseranfall ist im gleichnamigen entsprechenden Attribut anzugeben. / Nombre d''habitants dans le bassin versant direct, valeur à titre indicatif. Le débit d''eaux usées déterminant est spécifié dans l''attribut correspondant.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN population_dim  integer ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.population_dim IS 'yyy_Anzahl Einwohner im direkten Einzugsgebiet (Dimensionierung) als informativer Wert. Der massgebende Schmutzabwasseranfall ist im gleichnamigen entsprechenden Attribut anzugeben. / Anzahl Einwohner im direkten Einzugsgebiet (Dimensionierung) als informativer Wert. Der massgebende Schmutzabwasseranfall ist im gleichnamigen entsprechenden Attribut anzugeben. / Nombre d’habitants dans le bassin versant direct (dimensionnement), valeur indicative. Le débit des eaux usées doit être indiqué dans l’attribut du même nom.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN sewer_infiltration_water  decimal(9,3) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.sewer_infiltration_water IS 'yyy_Totaler Fremdwasseranfall beim Bauwerk inkl. aller obenliegenden Gebiete. Angabe Jahresmittelwert (24 Std.-Mittel) in l/s. / Totaler Fremdwasseranfall beim Bauwerk inkl. aller obenliegenden Gebiete. Angabe Jahresmittelwert (24 Std.-Mittel) in l/s. / Débit total d’eaux claires parasites à l’ouvrage, incluant les surfaces en amont. Moyenne annuelle sur 24 h en l/s.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_area  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_area IS 'yyy_Bruttofläche des direkten Einzugsgebietes im Misch- resp. Trennsystem gemäss Abbildung. / Bruttofläche des direkten Einzugsgebietes im Misch- resp. Trennsystem gemäss Abbildung. / Surface brute du bassin versant direct en système unitaire, resp. séparatif, selon illustration.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_dim  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_dim IS 'yyy_Bruttofläche des Einzugsgebiets Dimensionierung. Dieses Einzugsgebiet umfasst in der Regel alle obenliegenden Flächen des Regenbeckenüberlaufbeckens (inkl. denjenigen von Regenüberläufen, Pumpwerken, etc.) oder alle obenliegenden Flächen bis zum nächsten Regenüberlaufbecken. / Bruttofläche des Einzugsgebiets Dimensionierung. Dieses Einzugsgebiet umfasst in der Regel alle obenliegenden Flächen des Regenbeckenüberlaufbeckens (inkl. denjenigen von Regenüberläufen, Pumpwerken, etc.) oder alle obenliegenden Flächen bis zum nächsten Regenüberlaufbecken. / Surface brute du bassin versant de dimensionnement. Lors de la saisie des bassins d’eaux pluviales, il faut également indiquer le bassin versant de dimensionnement. Ce bassin versant contient toutes les surfaces en amont du BEP (incl. en amont les déversoirs d’orage, stations de pompage, etc.) jusqu''au prochain BEP.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_imp_dim  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_imp_dim IS 'yyy_Befestigte Fläche des Einzugsgebiets Dimensionierung im Misch- resp. Trennsystem (nur Regenüberlaufbecken). Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser angeschlossene befestigte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Befestigte Fläche des Einzugsgebiets Dimensionierung im Misch- resp. Trennsystem (nur Regenüberlaufbecken). Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser angeschlossene befestigte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Surface imperméabilisée du bassin versant de dimensionnement dans le système unitaire, resp. séparatif (BEP uniquement). Dans un système séparatif, il faut saisir dans la fiche technique la surface imperméabilisée raccordée aux eaux usées. Au minimum une surface (imperméabilisée ou réduite) doit être indiquée.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_imp_red  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_imp_red IS 'yyy_Impermeable suface des direkten Einzugsgebiets im Misch- resp. Trennsystem gemäss Abbildung. Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser ange-schlossene befestigte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Befestigte Fläche des direkten Einzugsgebiets im Misch- resp. Trennsystem gemäss Abbildung. Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser ange-schlossene befestigte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Surface imperméabilisée du bassin versant  direct pour un système unitaire, resp. séparatif selon illustration. Dans un système séparatif, il faut saisir dans la fiche technique la surface réduite raccordée aux eaux usées. Au minimum une surface (imperméabilisée ou réduite) doit être indiquée.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_red  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_red IS 'yyy_Reduzierte Fläche des direkten Einzugsgebiets im Misch- resp. Trennsystem gemäss Abbildung. Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser ange-schlossene reduzierte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Reduzierte Fläche des direkten Einzugsgebiets im Misch- resp. Trennsystem gemäss Abbildung. Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser ange-schlossene reduzierte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Surface réduite du bassin versant direct pour un système unitaire, resp. séparatif selon illustration. Dans un système séparatif, il faut saisir la surface réduite raccordée aux eaux usées. Au minimum une surface (imperméabilisée ou réduite) doit être indiquée.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN surface_red_dim  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.surface_red_dim IS 'yyy_Reduzierte Fläche des Einzugsgebiets Dimensionierung im Misch- resp. Trennsystem (nur Regenüberlaufbecken). Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser angeschlossene reduzierte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Reduzierte Fläche des Einzugsgebiets Dimensionierung im Misch- resp. Trennsystem (nur Regenüberlaufbecken). Im Trennsystem ist für die Stammkarte die an das Schmutzabwasser angeschlossene reduzierte Fläche anzugeben. Es muss mindestens eine Fläche (befestigt oder reduziert) angegeben werden. / Surface réduite du bassin versant de dimensionnement dans le système unitaire, resp. séparatif. Dans un système séparatif, il faut saisir la surface réduite raccordée aux eaux usées. Au minimum une surface (imperméabilisée ou réduite) doit être indiquée.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN waste_water_production  decimal(9,3) ;
COMMENT ON COLUMN qgep_od.catchement_area_totals.waste_water_production IS 'Total waste water production at construction of all areas above. Yearly average (of 24h average) in l/s. / Totaler Schmutzabwasseranfall beim Bauwerk inkl. aller obenliegenden Gebiete. Angabe Jahresmittelwert (24 Std.-Mittel) in l/s. / Débit total d’eaux usées à l’ouvrage, incluant les surfaces en amont. Moyenne annuelle sur 24 h en l/s.';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.catchement_area_totals.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.catchement_area_totals.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.catchement_area_totals.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
update_last_modified_catchement_area_totals
BEFORE UPDATE OR INSERT ON
 qgep_od.catchement_area_totals
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified();
...
COMMENT ON COLUMN qgep_od.param_ca_general.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
COMMENT ON COLUMN qgep_od.param_ca_mouse1.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
...
CREATE SEQUENCE qgep_od.seq_disposal_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.disposal ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','disposal');
COMMENT ON COLUMN qgep_od.disposal.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.disposal ADD COLUMN disposal_interval_current  decimal(4,2) ;
COMMENT ON COLUMN qgep_od.disposal.disposal_interval_current IS 'yyy_Abstände, in welchen das Bauwerk aktuell geleert wird (Jahre) / Abstände, in welchen das Bauwerk aktuell geleert wird (Jahre) / Fréquence à laquelle l''ouvrage subit actuellement une vidange (années)';
 ALTER TABLE qgep_od.disposal ADD COLUMN disposal_interval_nominal  decimal(4,2) ;
COMMENT ON COLUMN qgep_od.disposal.disposal_interval_nominal IS 'yyy_Abstände, in welchen das Bauwerk geleert werden sollte (Jahre); Vorgabe aus GEP / Abstände, in welchen das Bauwerk geleert werden sollte (Jahre); Vorgabe aus GEP / Fréquence à laquelle l''ouvrage devrait subir un vidange (années); exigence selon PGEE';
 ALTER TABLE qgep_od.disposal ADD COLUMN disposal_place_current  integer ;
COMMENT ON COLUMN qgep_od.disposal.disposal_place_current IS 'yyy_Ort der Schlammentsorgung im heutigen Zustand / Ort der Schlammentsorgung im heutigen Zustand / Lieu d''élimination des boues en état actuel';
 ALTER TABLE qgep_od.disposal ADD COLUMN disposal_place_planned  integer ;
COMMENT ON COLUMN qgep_od.disposal.disposal_place_planned IS 'yyy_Ort der Schlammentsorgung im Planungszustand (gemäss GEP) / Ort der Schlammentsorgung im Planungszustand (gemäss GEP) / Lieu d''élimination des boues en état planifié (selon PGEE)';
 ALTER TABLE qgep_od.disposal ADD COLUMN volume_pit_without_drain  decimal(9,2) ;
COMMENT ON COLUMN qgep_od.disposal.volume_pit_without_drain IS 'yyy_Abflusslose Grube: Stapelraum in m3 / Abflusslose Grube: Stapelraum in m3 / Fosse étanche: volume de stockage en m3';
 ALTER TABLE qgep_od.disposal ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.disposal.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.disposal ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.disposal.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.disposal ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.disposal.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
...
-------
-------
CREATE TABLE qgep_od.building_group
(
   obj_id varchar(16) NOT NULL,
...
CREATE SEQUENCE qgep_od.seq_building_group_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.building_group ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','building_group');
COMMENT ON COLUMN qgep_od.building_group.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.building_group ADD COLUMN camping_area  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.building_group.camping_area IS 'Camping: Area of camping in hectars / Camping: Fläche Campingplatz in ha / Camping: surface du camping en ha';
 ALTER TABLE qgep_od.building_group ADD COLUMN camping_lodgings  smallint ;
COMMENT ON COLUMN qgep_od.building_group.camping_lodgings IS 'Camping: Number of overnight stays per year / Camping: Anzahl Übernachtungen pro Jahr / Camping: nombre de nuitées par an';
 ALTER TABLE qgep_od.building_group ADD COLUMN church_seats  smallint ;
COMMENT ON COLUMN qgep_od.building_group.church_seats IS 'yyy_Kirche: Anzahl Sitzplätze (ohne Nebenräume) / Kirche: Anzahl Sitzplätze (ohne Nebenräume) / Eglise: nombre de places assises (sans pièces adjacentes)';
 ALTER TABLE qgep_od.building_group ADD COLUMN connecting_obligation  integer ;
COMMENT ON COLUMN qgep_od.building_group.connecting_obligation IS 'Defines whether the building must be connected to the public sewer system according to Art. 11 GschG and guidelines for wastewater disposal in rural areas. / Definiert, ob das Gebäude anschlusspflichtig an die öffentliche Kanalisation ist gemäss Art. 11 GschG und Leitfaden Abwasserentsorgung im ländlichen Raum / Définit si le bâtiment doit être raccordé aux égouts publics selon LEaux art. 11 et au guide VSA eaux usées en milieu rural.';
 ALTER TABLE qgep_od.building_group ADD COLUMN connection_wwtp  integer ;
COMMENT ON COLUMN qgep_od.building_group.connection_wwtp IS '';
 ALTER TABLE qgep_od.building_group ADD COLUMN craft_employees  smallint ;
COMMENT ON COLUMN qgep_od.building_group.craft_employees IS 'yyy_Verwaltungsgebäude, Geschäftshaus, Fabrik (ohne Industrieabwasser): Anzahl Beschäftigte / Verwaltungsgebäude, Geschäftshaus, Fabrik (ohne Industrieabwasser): Anzahl Beschäftigte / Entreprise (sans eaux usées industrielles): nombre d''employés';
 ALTER TABLE qgep_od.building_group ADD COLUMN dorm_beds  smallint ;
COMMENT ON COLUMN qgep_od.building_group.dorm_beds IS 'Dormatory number of beds / Schlafsaal: Anzahl Betten / Dortoir: nombre de lits';
 ALTER TABLE qgep_od.building_group ADD COLUMN dorm_overnight_stays  smallint ;
COMMENT ON COLUMN qgep_od.building_group.dorm_overnight_stays IS 'Dormatory overnight stays / Schlafsaal: Anzahl Übernachtungen pro Jahr / Dortoir: nombre de nuitées par an';
 ALTER TABLE qgep_od.building_group ADD COLUMN drainage_map  integer ;
COMMENT ON COLUMN qgep_od.building_group.drainage_map IS 'yyy_Angabe ob Pläne der Entwässerungsanlagen vorhanden / Angabe ob Pläne der Entwässerungsanlagen vorhanden / Indication si plans des installations d''assainissement existants';
 ALTER TABLE qgep_od.building_group ADD COLUMN drinking_water_network  integer ;
COMMENT ON COLUMN qgep_od.building_group.drinking_water_network IS 'yyy_Angabe ob Trinkwasseranschluss an öffentliches Netz vorhanden / Angabe ob Trinkwasseranschluss an öffentliches Netz vorhanden / Indication si raccordement au réseau public d''eau potable existant';
 ALTER TABLE qgep_od.building_group ADD COLUMN drinking_water_others  integer ;
COMMENT ON COLUMN qgep_od.building_group.drinking_water_others IS 'yyy_Andere Trinkwasserversorgung als Netzanschluss (Hauptversorgung oder zusätzlich zum Netzanschluss) / Andere Trinkwasserversorgung als Netzanschluss (Hauptversorgung oder zusätzlich zum Netzanschluss) / Autre alimentation en eau potable que raccordement au réseau (alimentation principale ou supplémentaire au raccordement au réseau)';
 ALTER TABLE qgep_od.building_group ADD COLUMN electric_connection  integer ;
COMMENT ON COLUMN qgep_od.building_group.electric_connection IS 'yyy_Angabe ob Anschluss an Stromversorgung vorhanden / Angabe ob Anschluss an Stromversorgung vorhanden / Indication si raccordement au réseau électrique existant';
 ALTER TABLE qgep_od.building_group ADD COLUMN event_visitors  smallint ;
COMMENT ON COLUMN qgep_od.building_group.event_visitors IS 'yyy_Veranstaltung: Anzahl Besucher / Maximale Anzahl Besucher pro Veranstaltung / Nombre maximum de visiteurs par manifestation';
 ALTER TABLE qgep_od.building_group ADD COLUMN function  integer ;
COMMENT ON COLUMN qgep_od.building_group.function IS 'Kind of building use / Art der Gebäudenutzung / Genre d''affectation des bâtiments';
 ALTER TABLE qgep_od.building_group ADD COLUMN gym_area  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.building_group.gym_area IS 'yyy_Turnhalle: Hallenfläche in m2 / Turnhalle: Hallenfläche in m2 / Salle de gymnastique: surface de salle en m2';
 ALTER TABLE qgep_od.building_group ADD COLUMN holiday_accomodation  smallint ;
COMMENT ON COLUMN qgep_od.building_group.holiday_accomodation IS 'yyy_Ausschliesslich Feriennutzung: Anzahl Übernachtungen pro Jahr / Ausschliesslich Feriennutzung: Anzahl Übernachtungen pro Jahr / Uniquement vacances: nombre de nuitées par an';
 ALTER TABLE qgep_od.building_group ADD COLUMN hospital_beds  smallint ;
COMMENT ON COLUMN qgep_od.building_group.hospital_beds IS 'Hospital, Nursing home: Number of beds / Spital, Pflegeanstalt: Anzahl Betten / Hôpital, EMS: nombre de lits';
 ALTER TABLE qgep_od.building_group ADD COLUMN hotel_beds  smallint ;
COMMENT ON COLUMN qgep_od.building_group.hotel_beds IS 'Hotel: Number of beds / Hotel: Anzahl Betten / Hôtel: nombre de lits';
 ALTER TABLE qgep_od.building_group ADD COLUMN hotel_overnight_stays  smallint ;
COMMENT ON COLUMN qgep_od.building_group.hotel_overnight_stays IS 'Hotel: Number of overnight stays per year / Hotel: Anzahl Übernachtungen pro Jahr / Hôtel: nombre de nuitées par an';
 ALTER TABLE qgep_od.building_group ADD COLUMN identifier  varchar(20) ;
COMMENT ON COLUMN qgep_od.building_group.identifier IS '';
 ALTER TABLE qgep_od.building_group ADD COLUMN movie_theater_seats  smallint ;
COMMENT ON COLUMN qgep_od.building_group.movie_theater_seats IS 'Cinema: Number of seats / Kino: Anzahl Sitzplätze / Cinéma: nombre de places assises';
 ALTER TABLE qgep_od.building_group ADD COLUMN other_usage_population_equivalent  integer ;
COMMENT ON COLUMN qgep_od.building_group.other_usage_population_equivalent IS 'yyy_Einwohnergleichwert für andere Art der Gebäudenutzung / Einwohnergleichwert für andere Art der Gebäudenutzung / Equivalent-habitants d''autre genre d''affectation de bâtiment';
 ALTER TABLE qgep_od.building_group ADD COLUMN other_usage_type  varchar(50) ;
COMMENT ON COLUMN qgep_od.building_group.other_usage_type IS 'yyy_Beschreibung für andere Art der Gebäudenutzung / Beschreibung für andere Art der Gebäudenutzung / Description d''autre genre d''affectation de bâtiment';
 ALTER TABLE qgep_od.building_group ADD COLUMN population_equivalent  integer ;
COMMENT ON COLUMN qgep_od.building_group.population_equivalent IS 'Consists of the number of inhabitants (E) and population equivalents (EGW). See VSA guideline ALR population equivalents A03 / Setzt sich aus der Einwohnerzahl (E) und Einwohnergleichwerten (EGW) zusammen. Siehe VSA Leitfaden ALR Einwohnerwerte A03 / Consiste en un nombre d''habitants (E) et d''équivalents de population (EGW). Voir la directive de la VSA sur les équivalents population EMR A03';
 ALTER TABLE qgep_od.building_group ADD COLUMN remark  varchar(80) ;
COMMENT ON COLUMN qgep_od.building_group.remark IS 'General remarks / Allgemeine Bemerkungen / Remarques générales';
 ALTER TABLE qgep_od.building_group ADD COLUMN renovation_date  timestamp without time zone ;
COMMENT ON COLUMN qgep_od.building_group.renovation_date IS '';
 ALTER TABLE qgep_od.building_group ADD COLUMN renovation_necessity  integer ;
COMMENT ON COLUMN qgep_od.building_group.renovation_necessity IS '';
 ALTER TABLE qgep_od.building_group ADD COLUMN restaurant_seats  smallint ;
COMMENT ON COLUMN qgep_od.building_group.restaurant_seats IS 'yyy_Stark frequentierte Gaststätte, wie Autobahnraststätte, Berggasthaus, etc.: Anzahl Sitzplätze / Stark frequentierte Gaststätte, wie Autobahnraststätte, Berggasthaus, etc.: Anzahl Sitzplätze / Restaurant très fréquenté, tel que restoroute, restaurant de montage, etc. : nombre de places assises';
 ALTER TABLE qgep_od.building_group ADD COLUMN restaurant_seats_hall_garden  smallint ;
COMMENT ON COLUMN qgep_od.building_group.restaurant_seats_hall_garden IS 'yyy_Restaurant: Anzahl Sitzplätze Säle und Garten / Restaurant: Anzahl Sitzplätze Säle und Garten / Restaurant: nombre de places salles et terrasses';
 ALTER TABLE qgep_od.building_group ADD COLUMN restaurant_seats_permanent  smallint ;
COMMENT ON COLUMN qgep_od.building_group.restaurant_seats_permanent IS 'yyy_Restaurant: Anzahl Sitzplätze (ohne Säle und Garten) / Restaurant: Anzahl Sitzplätze (ohne Säle und Garten) / Restaurant: nombre de places sans salles et terrasses';
 ALTER TABLE qgep_od.building_group ADD COLUMN restructuring_concept  varchar(255) ;
COMMENT ON COLUMN qgep_od.building_group.restructuring_concept IS 'Summary of the concept according to the service offer (GEP sample specification) / Kurzfassung des Konzepts gemäss Leistung Offerte (GEP Musterpflichtenheft) / Version courte du concept en fonction de l''offre de service (Cahier des charges type du PGEE)';
 ALTER TABLE qgep_od.building_group ADD COLUMN school_students  smallint ;
COMMENT ON COLUMN qgep_od.building_group.school_students IS 'School: Number of pupils / Schule: Anzahl Schüler / Ecole: nombre d''élèves';
ALTER TABLE qgep_od.building_group ADD COLUMN situation_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_building_group_situation_geometry ON qgep_od.building_group USING gist (situation_geometry );
COMMENT ON COLUMN qgep_od.building_group.situation_geometry IS 'Location of the building group (can be taken from data in the Eidg. Gebäude und Wohnungsregister BAU/GWR) / Lage der Gebäudegruppe (kann aus Daten Eidg. Gebäude und Wohnungsregister BAU/GWR übernommen werden) / Localisation du groupe de bâtiments (peut être repris des données du Registre fédéral des bâtiments et des logements (RegBL))';
 ALTER TABLE qgep_od.building_group ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.building_group.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.building_group ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.building_group.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.building_group ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.building_group.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
update_last_modified_building_group
BEFORE UPDATE OR INSERT ON
 qgep_od.building_group
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified();

-------
-------
CREATE TABLE qgep_od.building_group_baugwr
(
   obj_id varchar(16) NOT NULL,
...
CREATE SEQUENCE qgep_od.seq_building_group_baugwr_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.building_group_baugwr ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','building_group_baugwr');
COMMENT ON COLUMN qgep_od.building_group_baugwr.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.building_group_baugwr ADD COLUMN egid  integer ;
COMMENT ON COLUMN qgep_od.building_group_baugwr.egid IS 'yyy_EGID aus BAU/GWR der zur Gebäudegruppe gehörigen Gebäude / EGID aus BAU/GWR der zur Gebäudegruppe gehörigen Gebäude / EGID de BAU/RegBL des bâtiments appartenant au groupe de bâtiments';
 ALTER TABLE qgep_od.building_group_baugwr ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.building_group_baugwr.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.building_group_baugwr ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.building_group_baugwr.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.building_group_baugwr ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.building_group_baugwr.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
update_last_modified_building_group_baugwr
BEFORE UPDATE OR INSERT ON
 qgep_od.building_group_baugwr
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified();

-------
-------
CREATE TABLE qgep_od.farm
(
   obj_id varchar(16) NOT NULL,
...
CREATE SEQUENCE qgep_od.seq_farm_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.farm ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','farm');
COMMENT ON COLUMN qgep_od.farm.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.farm ADD COLUMN agriculture_aerable_surface  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.farm.agriculture_aerable_surface IS 'yyy_Landwirtschaftliche Nutzfläche in ha / Landwirtschaftliche Nutzfläche in ha / Surface agricole utile en ha';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_comment  varchar(100) ;
COMMENT ON COLUMN qgep_od.farm.cesspit_comment IS 'Further remarks cesspit volume / Weitere Anmerkungen zur Güllegrube / Remarques additionnel volume fosse à purin';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_volume  integer ;
COMMENT ON COLUMN qgep_od.farm.cesspit_volume IS 'yyy_Klassifizierung, ob das Volumen (teilweise) in einem Fremdbetrieb in der gleichen oder einer anderen Gemeinde vorhanden ist / Klassifizierung, ob das Volumen (teilweise) in einem Fremdbetrieb in der gleichen oder einer anderen Gemeinde vorhanden ist / Classification, si le volume est disponible (même partiellement) dans une exploitation externe dans la même commune ou dans une autre commune';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_volume_current  decimal(9,2) ;
COMMENT ON COLUMN qgep_od.farm.cesspit_volume_current IS 'yyy_Güllegrube: aktuell vorhandenes Volumen in m3 / Güllegrube: aktuell vorhandenes Volumen in m3 / Fosse à purin: volume actuel en m3';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_volume_nominal  decimal(9,2) ;
COMMENT ON COLUMN qgep_od.farm.cesspit_volume_nominal IS 'yyy_Güllegrube: erforderliches Volumen in m3  (Sollzustand); Vorgabe aus GEP / Güllegrube: erforderliches Volumen in m3  (Sollzustand); Vorgabe aus GEP / Fosse à purin: volume requis en m3; exigence selon PGEE';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_volume_ww_treated  decimal(9,2) ;
COMMENT ON COLUMN qgep_od.farm.cesspit_volume_ww_treated IS 'yyy_Güllegrube: erforderliches Volumen in m3, falls häusliches Abwasser separat behandelt würde / Güllegrube: erforderliches Volumen in m3, falls häusliches Abwasser separat behandelt würde / Fosse à purin: volume en m3 requis en cas de traitement séparé des eaux ménagères';
 ALTER TABLE qgep_od.farm ADD COLUMN cesspit_year_of_approval  smallint ;
COMMENT ON COLUMN qgep_od.farm.cesspit_year_of_approval IS 'yyy_Güllegrube: Bewilligungsjahr / Güllegrube: Bewilligungsjahr / Fosse à purin: année d''autorisation';
 ALTER TABLE qgep_od.farm ADD COLUMN conformity  integer ;
COMMENT ON COLUMN qgep_od.farm.conformity IS 'Conformity of Einrichtungen (Güllegrube, Mistplatz, etc.) / Konformität der Einrichtungen (Güllegrube, Mistplatz, etc.) / Conformité des installations (fosse à purin, fumière, etc.)';
 ALTER TABLE qgep_od.farm ADD COLUMN continuance  integer ;
COMMENT ON COLUMN qgep_od.farm.continuance IS 'yyy_Potentieller Fortbestand des Betriebs / Potentieller Fortbestand des Betriebs / Pérennité potentielle de l''exploitation';
 ALTER TABLE qgep_od.farm ADD COLUMN continuance_comment  varchar(80) ;
COMMENT ON COLUMN qgep_od.farm.continuance_comment IS 'yyy_Bemerkungen zum Fortbestand des Betriebs / Bemerkungen zum Fortbestand des Betriebs / Remarques concernant la pérennité de l''exploitation';
 ALTER TABLE qgep_od.farm ADD COLUMN muck_hill_area_current  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.farm.muck_hill_area_current IS 'yyy_Mistplatz: aktuell vorhandene Fläche in m2 / Mistplatz: aktuell vorhandene Fläche in m2 / Fumière: surface actuelle en m2';
 ALTER TABLE qgep_od.farm ADD COLUMN muck_hill_area_nominal  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.farm.muck_hill_area_nominal IS 'yyy_Mistplatz: erforderliche Fläche in m2 (Sollzustand); Vorgabe aus GEP / Mistplatz: erforderliche Fläche in m2 (Sollzustand); Vorgabe aus GEP / Fumière: surface requise en m2 ; exigence selon PGEE';
 ALTER TABLE qgep_od.farm ADD COLUMN remark  varchar(80) ;
COMMENT ON COLUMN qgep_od.farm.remark IS 'General remarks / Allgemeine Bemerkungen / Remarques générales';
 ALTER TABLE qgep_od.farm ADD COLUMN shepherds_hut_comment  varchar(80) ;
COMMENT ON COLUMN qgep_od.farm.shepherds_hut_comment IS 'yyy_Hirtenhütte: Bemerkung betreffend Abwasserproduktion / Hirtenhütte: Bemerkung betreffend Abwasserproduktion / Rural / remise: remarque concernant production d''eau usée';
 ALTER TABLE qgep_od.farm ADD COLUMN shepherds_hut_population_equivalent  integer ;
COMMENT ON COLUMN qgep_od.farm.shepherds_hut_population_equivalent IS 'yyy_Hirtenhütte: Einwohnergleichwert / Hirtenhütte: Einwohnergleichwert / Rural / remise: équivalent-habitants';
 ALTER TABLE qgep_od.farm ADD COLUMN shepherds_hut_wastewater  integer ;
COMMENT ON COLUMN qgep_od.farm.shepherds_hut_wastewater IS 'yyy_Hirtenhütte: Fällt häusliches Abwasser an? / Hirtenhütte: Fällt häusliches Abwasser an? / Rural / remise: production d''eau usée?';
 ALTER TABLE qgep_od.farm ADD COLUMN stable_cattle  integer ;
COMMENT ON COLUMN qgep_od.farm.stable_cattle IS 'yyy_Stall: Vieh vorhanden? / Stall: Vieh vorhanden? / Etable / écurie: bétail existant?';
 ALTER TABLE qgep_od.farm ADD COLUMN stable_cattle_equivalent_other_cattle  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.farm.stable_cattle_equivalent_other_cattle IS 'yyy_Stall: Anzahl Tiere in Düngergrossvieheinheiten DGVE (Fremdvieh) fertiliser produced per livestock unit (FLU) / Stall: Anzahl Tiere in Düngergrossvieheinheiten DGVE (Fremdvieh) / Etable / écurie: nombre d''animaux exprimés en unité de gros bétail UGB (bétail "étranger")';
 ALTER TABLE qgep_od.farm ADD COLUMN stable_cattle_equivalent_own_cattle  decimal(8,2) ;
COMMENT ON COLUMN qgep_od.farm.stable_cattle_equivalent_own_cattle IS 'yyy_Stall: Anzahl Tiere in Düngergrossvieheinheiten DGVE (eigenes Vieh) fertiliser produced per livestock unit (FLU) / Stall: Anzahl Tiere in Düngergrossvieheinheiten DGVE (eigenes Vieh) / Etable / écurie: nombre d''animaux exprimés en unité de gros bétail UGB (propre bétail)';
 ALTER TABLE qgep_od.farm ADD COLUMN last_modification TIMESTAMP without time zone DEFAULT now();
COMMENT ON COLUMN qgep_od.farm.last_modification IS 'Last modification / Letzte_Aenderung / Derniere_modification: INTERLIS_1_DATE';
 ALTER TABLE qgep_od.farm ADD COLUMN fk_dataowner varchar(16);
COMMENT ON COLUMN qgep_od.farm.fk_dataowner IS 'Foreignkey to Metaattribute dataowner (as an organisation) - this is the person or body who is allowed to delete, change or maintain this object / Metaattribut Datenherr ist diejenige Person oder Stelle, die berechtigt ist, diesen Datensatz zu löschen, zu ändern bzw. zu verwalten / Maître des données gestionnaire de données, qui est la personne ou l''organisation autorisée pour gérer, modifier ou supprimer les données de cette table/classe';
 ALTER TABLE qgep_od.farm ADD COLUMN fk_provider varchar(16);
COMMENT ON COLUMN qgep_od.farm.fk_provider IS 'Foreignkey to Metaattribute provider (as an organisation) - this is the person or body who delivered the data / Metaattribut Datenlieferant ist diejenige Person oder Stelle, die die Daten geliefert hat / FOURNISSEUR DES DONNEES Organisation qui crée l’enregistrement de ces données ';
-------
CREATE TRIGGER
...
-------
-------
CREATE TABLE qgep_od.small_treatment_plant
(
   obj_id varchar(16) NOT NULL,
...
CREATE SEQUENCE qgep_od.seq_small_treatment_plant_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.small_treatment_plant ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','small_treatment_plant');
COMMENT ON COLUMN qgep_od.small_treatment_plant.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.small_treatment_plant ADD COLUMN approval_number  varchar(50) ;
COMMENT ON COLUMN qgep_od.small_treatment_plant.approval_number IS 'yyy_Bewilligungsnummer der Aufsichtsbehörde / Bewilligungsnummer der Aufsichtsbehörde / Numéro d''autorisation de l''autorité de surveillance';
 ALTER TABLE qgep_od.small_treatment_plant ADD COLUMN function  integer ;
COMMENT ON COLUMN qgep_od.small_treatment_plant.function IS 'yyy_Art des Verfahrens / Art des Verfahrens / Genre de procédé';
 ALTER TABLE qgep_od.small_treatment_plant ADD COLUMN installation_number  integer ;
COMMENT ON COLUMN qgep_od.small_treatment_plant.installation_number IS 'yyy_ARA-Nummer gemäss BAFU / ARA-Nummer gemäss BAFU / Numéro de la STEP selon l''OFEV';
 ALTER TABLE qgep_od.small_treatment_plant ADD COLUMN remote_monitoring  integer ;
COMMENT ON COLUMN qgep_od.small_treatment_plant.remote_monitoring IS '';
-------
CREATE TRIGGER
update_last_modified_small_treatment_plant
BEFORE UPDATE OR INSERT ON
 qgep_od.small_treatment_plant
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified_parent("qgep_od.wastewater_structure");

-------
-------
CREATE TABLE qgep_od.drainless_toilet
(
   obj_id varchar(16) NOT NULL,
   CONSTRAINT pkey_qgep_od_drainless_toilet_obj_id PRIMARY KEY (obj_id)
)
WITH (
   OIDS = False
);
CREATE SEQUENCE qgep_od.seq_drainless_toilet_oid INCREMENT 1 MINVALUE 0 MAXVALUE 999999 START 0;
 ALTER TABLE qgep_od.drainless_toilet ALTER COLUMN obj_id SET DEFAULT qgep_sys.generate_oid('qgep_od','drainless_toilet');
COMMENT ON COLUMN qgep_od.drainless_toilet.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.drainless_toilet ADD COLUMN kind  integer ;
COMMENT ON COLUMN qgep_od.drainless_toilet.kind IS '';
-------
CREATE TRIGGER
update_last_modified_drainless_toilet
BEFORE UPDATE OR INSERT ON
 qgep_od.drainless_toilet
FOR EACH ROW EXECUTE PROCEDURE
 qgep_sys.update_last_modified_parent("qgep_od.wastewater_structure");
...
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.text_plantype ADD CONSTRAINT pkey_qgep_vl_text_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7844,7844,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7846,7846,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7847,7847,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7848,7848,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7845,7845,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.symbol_plantype ADD CONSTRAINT pkey_qgep_vl_symbol_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7874,7874,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7876,7876,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7877,7877,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7878,7878,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7875,7875,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.progression_alternative ADD COLUMN fk_reach varchar(16);
ALTER TABLE qgep_od.progression_alternative ADD CONSTRAINT rel_progression_alternative_reach FOREIGN KEY (fk_reach) REFERENCES qgep_od.reach(obj_id) ON UPDATE CASCADE ON DELETE cascade;
CREATE TABLE qgep_vl.progression_alternative_plantype () INHERITS (qgep_sys.value_list_base);

--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.progression_alternative_plantype ADD CONSTRAINT pkey_qgep_vl_progression_alternative_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.progression_alternative_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9282,9282,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.progression_alternative_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9285,9285,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.progression_alternative_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9286,9286,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.progression_alternative_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9287,9287,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.progression_alternative_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9284,9284,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.progression_alternative ADD CONSTRAINT fkey_vl_progression_alternative_plantype FOREIGN KEY (plantype)
 REFERENCES qgep_vl.progression_alternative_plantype (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.organisation_organisation_type () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.organisation_organisation_type ADD CONSTRAINT pkey_qgep_vl_organisation_organisation_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8608,8608,'waste_water_association','Abwasserverband','association_epuration_eau', 'consorzio_depurazione', 'rrr_Abwasserverband', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8715,8715,'federation','Bund','federation', 'confederazione', 'rrr_Bund', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8604,8604,'municipality','Gemeinde','commune', 'comune', 'rrr_Gemeinde', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8610,8610,'cooperative','Genossenschaft_Korporation','cooperative', 'cooperativa_corporazione', 'rrr_Genossenschaft_Korporation', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8605,8605,'canton','Kanton','canton', 'cantone', 'rrr_Kanton', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_organisation_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8606,8606,'private','Privat','prive', 'privato', 'privata', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.organisation ADD CONSTRAINT fkey_vl_organisation_organisation_type FOREIGN KEY (organisation_type)
 REFERENCES qgep_vl.organisation_organisation_type (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.organisation_status () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.organisation_status ADD CONSTRAINT pkey_qgep_vl_organisation_status_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.organisation_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9047,9047,'activ','aktiv','active', 'attivo', 'rrr_aktiv', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.organisation_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9048,9048,'gone','untergegangen','disparue', 'decaduta', 'rrr_untergegangen', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.organisation ADD CONSTRAINT fkey_vl_organisation_status FOREIGN KEY (status)
 REFERENCES qgep_vl.organisation_status (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.measure ADD COLUMN fk_responsible_entity varchar(16);
ALTER TABLE qgep_od.measure ADD CONSTRAINT rel_measure_responsible_entity FOREIGN KEY (fk_responsible_entity) REFERENCES qgep_od.organisation(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.measure ADD COLUMN fk_responsible_start varchar(16);
ALTER TABLE qgep_od.measure ADD CONSTRAINT rel_measure_responsible_start FOREIGN KEY (fk_responsible_start) REFERENCES qgep_od.organisation(obj_id) ON UPDATE CASCADE ON DELETE set null;
CREATE TABLE qgep_vl.measure_category () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.measure_category ADD CONSTRAINT pkey_qgep_vl_measure_category_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8649,8649,'runoff_prevention_retention_infiltration','Abflussvermeidung_Retention_Versickerung','limitation_ecoulement_retention_infiltration', 'limitazione_deflusso_ritenzione_infiltrazione', 'rrr_Abflussvermeidung_Retention_Versickerung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4653,4653,'administrative_mesure','administrative_Massnahme','mesure_administrative', 'misura_amministrativa', 'rrr_administrative_Massnahme', 'G', 'G', 'G', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9144,9144,'WWDRA','ALR','EMR', 'smaltimento_acque_zone_rurali', 'rrr_ALR', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5036,5036,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4654,4654,'abolishment','Aufhebung','suppression', 'soppressione', '', 'G', 'G', 'G', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4657,4657,'datamanagement','Datenmanagement','gestion_des_donnees', 'gestione_dati', 'rrr_Datenmanagement', 'D', 'D', 'D', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8706,8706,'maintenance_replacement','Erhaltung_Erneuerung','maintenance_renouvellement', 'manutenzione_rinnovo', 'rrr_Erhaltung_Erneuerung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8648,8648,'maintenance_cleaning','Erhaltung_Reinigung','maintenance_nettoyage', 'manutenzione_pulizia', 'rrr_Erhaltung_Reinigung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8646,8646,'maintenance_renovation_repair','Erhaltung_Renovierung_Reparatur','maintenance_renovation_reparation', 'manutenzione_rinnovo_riparazione', 'rrr_Erhaltung_Renovierung_Reparatur', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8647,8647,'maintenance_unknown','Erhaltung_unbekannt','maintenance_inconnue', 'manutenzione_sconosciuto', 'rrr_Erhaltung_unbekannt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4659,4659,'sewer_infiltration_water_reduction','Fremdwasserreduktion','reduction_ecp', 'riduzione_acque_chiare', 'rrr_Fremdwasserreduktion', '', 'G', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8645,8645,'function_change','Funktionsaenderung','changement_de_fonction', 'modifica_funzione', 'rrr_Funktionsaenderung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4660,4660,'gwdp_elaboration','GEP_Bearbeitung','elaboration_PGEE', 'elaborazione_PGS', 'rrr_GEP_Bearbeitung', '', 'G', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4662,4662,'control_and_surveillance','Kontrolle_und_Ueberwachung','controle_et_surveillence', 'controllo_monitoraggio', 'rrr_Kontrolle_und_Ueberwachung', '', 'G', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8639,8639,'measures_in_water_body','Massnahme_im_Gewaesser','mesure_sur_cours_d_eau', 'intervento_in_corso_d_acqua', 'rrr_Massnahme_im_Gewaesser', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4666,4666,'network_extension','Netzerweiterung','extension_reseau', 'estensione_rete', 'rrr_Netzerweiterung', '', 'G', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8707,8707,'special_construction_customization','Sonderbauwerk_Anpassung','ouvrage_special_adaptation', 'manufatto_speciale_modifica', 'rrr_Sonderbauwerk_Anpassung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8705,8705,'special_construction_new_buildung','Sonderbauwerk_Neubau','ouvrage_special_nouvelle_construction', 'manufatto_speciale_nuova_costruzione', 'rrr_Sonderbauwerk_Neubau', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8650,8650,'incident_prevention','Stoerfallvorsorge','prevention_des_accidents_majeurs', 'prevenzione_incidenti_rilevanti', 'rrr_Stoerfallvorsorge', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_category (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4652,4652,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', 'U', 'I', '', '', 'true');
 ALTER TABLE qgep_od.measure ADD CONSTRAINT fkey_vl_measure_category FOREIGN KEY (category)
 REFERENCES qgep_vl.measure_category (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.measure_priority () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.measure_priority ADD CONSTRAINT pkey_qgep_vl_measure_priority_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4759,4759,'M0','M0','M0', 'M0', 'M0', 'M0', 'M0', 'M0', '', '', 'true');
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4760,4760,'M1','M1','M1', 'M1', 'M1', 'M1', 'M1', 'M1', '', '', 'true');
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4761,4761,'M2','M2','M2', 'M2', 'M2', 'M2', 'M2', 'M2', '', '', 'true');
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4762,4762,'M3','M3','M3', 'M3', 'M3', 'M3', 'M3', 'M3', '', '', 'true');
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4763,4763,'M4','M4','M4', 'M4', 'M4', 'M4', 'M4', 'M4', '', '', 'true');
 INSERT INTO qgep_vl.measure_priority (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5584,5584,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.measure ADD CONSTRAINT fkey_vl_measure_priority FOREIGN KEY (priority)
 REFERENCES qgep_vl.measure_priority (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.measure_status () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.measure_status ADD CONSTRAINT pkey_qgep_vl_measure_status_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4764,4764,'completed','erledigt','regle', 'completato', 'rrr_erledigt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4765,4765,'in_preparation','in_Bearbeitung','en_preparation', 'in_allestimento', 'rrr_in_Bearbeitung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4766,4766,'pending','pendent','en_suspens', 'in_attesa', 'rrr_pendent', 'in_', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4767,4767,'suspended','sistiert','supprime', 'sospesa', 'rrr_sistiert', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4768,4768,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.measure ADD CONSTRAINT fkey_vl_measure_status FOREIGN KEY (status)
 REFERENCES qgep_vl.measure_status (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.mutation_kind ADD CONSTRAINT pkey_qgep_vl_mutation_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.mutation_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5523,5523,'created','erstellt','cree', 'creato', 'creat', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.mutation_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5582,5582,'changed','geaendert','changee', 'modificato', 'modificat', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.mutation_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5583,5583,'deleted','geloescht','effacee', 'eliminata', 'eliminate', '', '', '', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.waste_water_treatment_plant_operator_type () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.waste_water_treatment_plant_operator_type ADD CONSTRAINT pkey_qgep_vl_waste_water_treatment_plant_operator_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8986,8986,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8978,8978,'idividual_municipality','Einzelgemeinde','commune_individuelle', 'zzz_Einzelgemeinde', 'rrr_Einzelgemeinde', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8981,8981,'intermunicipal_agency','Interkommunale_Anstalt','etablissement_intercommunal', 'zzz_Interkommunale_Anstalt', 'rrr_Interkommunale_Anstalt', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8984,8984,'private_company','Privates_Unternehmen','entreprise_privee', 'zzz_Privates_Unternehmen', 'rrr_Privates_Unternehmen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8980,8980,'domicile_municipality','Sitzgemeinde','commune_d_implantation', 'zzz_Sitzgemeinde', 'rrr_Sitzgemeinde', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_plant_operator_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8979,8979,'special_purpose_association','Zweckverband','groupement', 'zzz_Zweckverband', 'rrr_Zweckverband', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.waste_water_treatment_plant ADD CONSTRAINT fkey_vl_waste_water_treatment_plant_operator_type FOREIGN KEY (operator_type)
 REFERENCES qgep_vl.waste_water_treatment_plant_operator_type (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_od.wastewater_structure ADD CONSTRAINT rel_wastewater_structure_main_cover FOREIGN KEY (fk_main_cover) REFERENCES qgep_od.cover(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.wastewater_structure_renovation_necessity ADD CONSTRAINT pkey_qgep_vl_wastewater_structure_renovation_necessity_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.wastewater_structure_renovation_necessity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9295,9295,'long_term','langfristig','a_long_terme', 'lungo_termine', 'termen_lung', 'LT', 'LF', 'LT', 'LT', 'TL', 'true');
...
ALTER TABLE qgep_vl.wastewater_structure_status ADD CONSTRAINT pkey_qgep_vl_wastewater_structure_status_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.wastewater_structure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3027,3027,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', 'U', 'I', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6526,6526,'other.calculation_alternative','weitere.Berechnungsvariante','autre.variante_de_calcul', 'altro.variante_calcolo', 'alta.varianta_calcul', 'CA', 'B', 'C', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7959,7959,'other.planned','weitere.geplant','autre.planifie', 'altro.previsto', 'rrr_weitere.geplant', '', 'G', 'PL', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6529,6529,'other.project','weitere.Projekt','autre.projet', 'altro.progetto', 'alta.proiect', '', 'N', 'PR', '', '', 'true');
...
ALTER TABLE qgep_vl.wastewater_structure_structure_condition ADD CONSTRAINT pkey_qgep_vl_wastewater_structure_structure_condition_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.wastewater_structure_structure_condition (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3362,3362,'Z4','Z4','Z4', 'Z4', 'Z4', '', 'Z4', 'Z4', '', '', 'true');
...
ALTER TABLE qgep_vl.channel_connection_type ADD CONSTRAINT pkey_qgep_vl_channel_connection_type_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.channel_function_amelioration () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.channel_function_amelioration ADD CONSTRAINT pkey_qgep_vl_channel_function_amelioration_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.channel_function_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4582,4582,'main_sewer','Hauptkanal','collecteur_principal', 'zzz_Hauptkanal', 'rrr_Hauptkanal', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_function_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4583,4583,'collector_sewer','Sammelkanal','collecteur', 'zzz_Sammelkanal', 'rrr_Sammelkanal', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_function_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4584,4584,'suction_pipe','Sauger','drains', 'zzz_Sauger', 'rrr_Sauger', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_function_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4585,4585,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.channel ADD CONSTRAINT fkey_vl_channel_function_amelioration FOREIGN KEY (function_amelioration)
 REFERENCES qgep_vl.channel_function_amelioration (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.channel_function_hydraulic ADD CONSTRAINT pkey_qgep_vl_channel_function_hydraulic_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.channel_function_hydraulic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8662,8662,'infiltration_pipe','Versickerungsleitung','conduite_d_infiltration', 'condotta_d_infiltrazione', 'rrr_Versickerungsleitung', '', '', '', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.channel_seepage () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.channel_seepage ADD CONSTRAINT pkey_qgep_vl_channel_seepage_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.channel_seepage (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4793,4793,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_seepage (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4794,4794,'wood_chips','Holzschnitzel','copeaux_bois', 'zzz_Holzschnitzel', 'rrr_Holzschnitzel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_seepage (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4795,4795,'soakaway_gravel','Sickerkies','gravier', 'zzz_Sickerkies', 'rrr_Sickerkies', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.channel_seepage (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4796,4796,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.channel ADD CONSTRAINT fkey_vl_channel_seepage FOREIGN KEY (seepage)
 REFERENCES qgep_vl.channel_seepage (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.channel_usage_current ADD CONSTRAINT pkey_qgep_vl_channel_usage_current_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.channel_usage_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9023,9023,'surface_water','Niederschlagsabwasser','eaux_de_surface', 'acque_piovane', 'apa_meteorica', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.channel_usage_planned ADD CONSTRAINT pkey_qgep_vl_channel_usage_planned_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.channel_usage_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9022,9022,'surface_water','Niederschlagsabwasser','eaux_de_surface', 'acque_piovane', 'apa_meteorica', '', '', '', '', '', 'true');
...
CREATE TABLE qgep_vl.manhole_amphibian_exit () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.manhole_amphibian_exit ADD CONSTRAINT pkey_qgep_vl_manhole_amphibian_exit_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.manhole_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9052,9052,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9053,9053,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9054,9054,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.manhole ADD CONSTRAINT fkey_vl_manhole_amphibian_exit FOREIGN KEY (amphibian_exit)
 REFERENCES qgep_vl.manhole_amphibian_exit (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.manhole_function ADD CONSTRAINT pkey_qgep_vl_manhole_function_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8702,8702,'treatment_plant','Behandlungsanlage','installation_traitement', 'installazione_di_trattamento', 'rrr_Behandlungsanlage', '', 'BH', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8995,8995,'floor_drain','Bodenablauf','ecoulement_de_sol', 'scarico_a_pavimento', 'rrr_Bodenablauf', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3267,3267,'rain_water_manhole','Dachwasserschacht','chambre_recolte_eaux_toitures', 'pozzetto_acque_tetti', 'camin_vizitare_apa_meteorica', 'RWM', 'DS', 'CRT', '', '', 'true');
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3266,3266,'gully','Einlaufschacht','chambre_avec_grille_d_entree', 'pozzetto_griglia', 'gura_scurgere', 'G', 'ES', 'CG', '', '', 'true');
 ...
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8828,8828,'drainage_channel_with_mud_bag','Entwaesserungsrinne_mit_Schlammsack','rigole_de_drainage_avec_depotoir', 'zzz_canaletta_drenaggio_mit_Schlammsack', 'rrr_Entwaesserungsrinne_mit_Schlammsack', '', 'ERS', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8601,8601,'grease_separator','Fettabscheider','separateur_de_graisse', 'separatore_di_grasso', 'rrr_Fettabscheider', '', 'FA', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8654,8654,'combined_manhole','Kombischacht','chambre_combine', 'pozzetto_doppio', 'rrr_Kombischacht', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8736,8736,'manhole','Kontroll_Einsteigschacht','chambre_de_visite_ou_d_inspection', 'pozzetto_di_ispezione', 'rrr_Kontroll_Einsteigschacht', '', 'KS', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5346,5346,'stormwater_overflow','Regenueberlauf','deversoir_d_orage', 'scaricatore_piena', 'preaplin', 'SO', 'RU', 'DO', 'SP', '', 'true');
 ...
 INSERT INTO qgep_vl.manhole_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8703,8703,'pretreatment_plant','Vorbehandlungsanlage','installation_de_pretraitement', 'zzz_Vorbehandlungsanlage', 'rrr_Vorbehandlungsanlage', 'yyy', 'VH', '', '', '', 'true');
...
ALTER TABLE qgep_vl.manhole_material ADD CONSTRAINT pkey_qgep_vl_manhole_material_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.manhole_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4541,4541,'concrete','Beton','beton', 'calcestruzzo', 'beton', '', '', '', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.manhole_possibility_intervention () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.manhole_possibility_intervention ADD CONSTRAINT pkey_qgep_vl_manhole_possibility_intervention_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.manhole_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9056,9056,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9057,9057,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.manhole_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9058,9058,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.manhole ADD CONSTRAINT fkey_vl_manhole_possibility_intervention FOREIGN KEY (possibility_intervention)
 REFERENCES qgep_vl.manhole_possibility_intervention (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
CREATE TABLE qgep_vl.special_structure_amphibian_exit () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.special_structure_amphibian_exit ADD CONSTRAINT pkey_qgep_vl_special_structure_amphibian_exit_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9052,9052,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9053,9053,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_amphibian_exit (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9054,9054,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.special_structure ADD CONSTRAINT fkey_vl_special_structure_amphibian_exit FOREIGN KEY (amphibian_exit)
 REFERENCES qgep_vl.special_structure_amphibian_exit (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.special_structure_bypass ADD CONSTRAINT pkey_qgep_vl_special_structure_bypass_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_bypass (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2682,2682,'inexistent','nicht_vorhanden','inexistant', 'non_presente', 'inexistent', '', 'NV', 'IE', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_bypass (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2681,2681,'existent','vorhanden','existant', 'presente', 'existent', '', 'V', 'E', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.special_structure_emergency_overflow () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.special_structure_emergency_overflow ADD CONSTRAINT pkey_qgep_vl_special_structure_emergency_overflow_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5866,5866,'other','andere','autres', 'altro', 'altele', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9075,9075,'in_water_body','in_Gewaesser','dans_cours_d_eau', 'in_corso_d_acqua', 'in_curs_apa', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9077,9077,'in_combined_waste_water_drain','in_Mischabwasserkanalisation','dans_canalisation_eaux_mixtes', 'in_canalizzazione_acque_miste', 'in_canalizare_ape_mixte', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9078,9078,'in_rain_waste_water_drain','in_Regenabwasserkanalisation','dans_canalisation_eaux_pluviales', 'in_canalizzazione_acque_meteoriche', 'in_canalizare_apa_meteorica', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9079,9079,'in_waste_water_drain','in_Schmutzabwasserkanalisation','dans_canalisation_eaux_usees', 'in_canalizzazione_acque_luride', 'in_canalizare_apa_uzata', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5878,5878,'none','keiner','aucun', 'nessuno', 'niciunul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9076,9076,'surface_discharge','oberflaechlich_ausmuendend','deversement_en_surface', 'con_sbocco_superficiale', 'deversare_la_suprafata', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5867,5867,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscut', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.special_structure ADD CONSTRAINT fkey_vl_special_structure_emergency_overflow FOREIGN KEY (emergency_overflow)
 REFERENCES qgep_vl.special_structure_emergency_overflow (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.special_structure_function ADD CONSTRAINT pkey_qgep_vl_special_structure_function_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6397,6397,'pit_without_drain','abflussloseGrube','fosse_etanche', 'fossa_senza_scarico', 'bazin_vidanjabil', '', 'AG', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6398,6398,'hydrolizing_tank','Abwasserfaulraum','fosse_digestive', 'camera_fermentazione', 'fosa_hidroliza', '', 'AF', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8704,8704,'treatment_plant','Behandlungsanlage','installation_traitement', 'installazione_di_trattamento', 'rrr_Behandlungsanlage', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3234,3234,'inverse_syphon_chamber','Duekerkammer','chambre_avec_siphon_inverse', 'camera_sifone', 'instalatie_cu_sifon_inversat', 'ISC', 'DK', 'SI', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6399,6399,'septic_tank_two_chambers','Faulgrube','fosse_septique_2_compartiments', 'zzz_Faulgrube', 'fosa_septica_2_compartimente', '', 'FG', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8600,8600,'grease_separator','Fettabscheider','separateur_de_graisse', 'separatore_di_grasso', 'rrr_Fettabscheider', '', 'FA', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5494,5494,'cesspit','Guellegrube','fosse_a_purin', 'fossa_liquame', 'hazna', '', 'JG', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8657,8657,'emergency_basin','Havariebecken','bassin_d_avarie', 'bacino_avaria', 'rrr_Havariebecken', '', 'HB', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6478,6478,'septic_tank','Klaergrube','fosse_septique', 'fossa_settica', 'fosa_septica', '', 'KG', 'FD', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8739,8739,'manhole','Kontroll_Einsteigschacht','chambre_de_visite_ou_d_inspection', 'pozzetto_di_ispezione', 'rrr_Kontroll_Einsteigschacht', '', 'KS', 'RV', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3677,3677,'stormwater_composite_tank','Regenbecken_Verbundbecken','BEP_combine', 'bacino_combinato', 'rrr_Regenbecken_Verbundbecken', '', 'VB', 'BCO', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (383,383,'side_access','seitlicherZugang','acces_lateral', 'accesso_laterale', 'acces_lateral', '', 'SZ', 'AL', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4799,4799,'separating_structure','Trennbauwerk','ouvrage_de_repartition', 'camera_ripartizione', 'separator', '', 'TS', 'OR', '', '', 'true');
 ...
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9089,9089,'pretreatment_plant','Vorbehandlungsanlage','installation_de_pretraitement', 'zzz_Vorbehandlungsanlage', 'rrr_Vorbehandlungsanlage', '', 'VH', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2745,2745,'vortex_manhole','Wirbelfallschacht','chambre_de_chute_a_vortex', 'camera_caduta_vortice', 'instalatie_picurare_cu_vortex', '', 'WF', 'CT', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.special_structure_possibility_intervention () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.special_structure_possibility_intervention ADD CONSTRAINT pkey_qgep_vl_special_structure_possibility_intervention_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9056,9056,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9057,9057,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_possibility_intervention (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9058,9058,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.special_structure ADD CONSTRAINT fkey_vl_special_structure_possibility_intervention FOREIGN KEY (possibility_intervention)
 REFERENCES qgep_vl.special_structure_possibility_intervention (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.special_structure_stormwater_tank_arrangement ADD CONSTRAINT pkey_qgep_vl_special_structure_stormwater_tank_arrangement_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.special_structure_stormwater_tank_arrangement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4608,4608,'main_connection','Hauptschluss','en_serie', 'in_serie', 'conectare_directa', '', 'HS', 'CD', '', '', 'true');
 INSERT INTO qgep_vl.special_structure_stormwater_tank_arrangement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4609,4609,'side_connection','Nebenschluss','en_parallele', 'in_parallelo', 'conectare_laterala', '', 'NS', 'CL', '', '', 'true');
...
ALTER TABLE qgep_vl.infiltration_installation_defects ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_defects_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.infiltration_installation_emergency_overflow () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.infiltration_installation_emergency_overflow ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_emergency_overflow_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9069,9069,'other','andere','autre', 'altro', 'rrr_andere', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9074,9074,'in_water_body','in_Gewaesser','dans_cours_d_eau', 'zzz_in_Gewaesser', 'in_curs_apa', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9070,9070,'in_combined_waste_water_drain','in_Mischabwasserkanalisation','dans_canalisation_eaux_mixtes', 'in_canalizzazione_acque_miste', 'in_canalizare_ape_mixte', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9072,9072,'in_rain_waste_water_drain','in_Regenabwasserkanalisation','dans_canalisation_eaux_pluviales', 'in_canalizzazione_acque_meteoriche', 'in_canalizare_apa_meteorica', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9073,9073,'in_waste_water_drain','in_Schmutzabwasserkanalisation','dans_canalisation_eaux_usees', 'in_canalizzazione_acque_luride', 'in_canalizare_apa_uzata', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3303,3303,'none','keiner','aucun', 'nessuno', 'inexistent', '', 'K', 'AN', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9071,9071,'surface_discharge','oberflaechlich_ausmuendend','deversement_en_surface', 'zzz_oberflaechlich_ausmuendend', 'deversare_la_suprafata', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_emergency_overflow (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3308,3308,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscut', '', 'U', 'I', '', '', 'true');
 ALTER TABLE qgep_od.infiltration_installation ADD CONSTRAINT fkey_vl_infiltration_installation_emergency_overflow FOREIGN KEY (emergency_overflow)
 REFERENCES qgep_vl.infiltration_installation_emergency_overflow (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.infiltration_installation_filling_material () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.infiltration_installation_filling_material ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_filling_material_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.infiltration_installation_filling_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4785,4785,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_filling_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4786,4786,'wood_chips','Holzschnitzel','copeaux_bois', 'zzz_Holzschnitzel', 'rrr_Holzschnitzel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_filling_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4787,4787,'soakaway_gravel','Sickerkies','gravier', 'zzz_Sickerkies', 'rrr_Sickerkies', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_filling_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4788,4788,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.infiltration_installation ADD CONSTRAINT fkey_vl_infiltration_installation_filling_material FOREIGN KEY (filling_material)
 REFERENCES qgep_vl.infiltration_installation_filling_material (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.infiltration_installation_kind ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.infiltration_installation_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3282,3282,'with_soil_passage','andere_mit_Bodenpassage','autre_avec_passage_a_travers_sol', 'zzz_altro_mit_Bodenpassage', 'altul_cu_traversare_sol', 'WSP', 'AMB', 'APC', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_installation_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3285,3285,'without_soil_passage','andere_ohne_Bodenpassage','autre_sans_passage_a_travers_sol', 'zzz_altro_ohne_Bodenpassage', 'altul_fara_traversare_sol', 'WOP', 'AOB', 'ASC', '', '', 'true');
...
ALTER TABLE qgep_vl.infiltration_installation_seepage_utilization ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_seepage_utilization_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.infiltration_installation_seepage_utilization (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9024,9024,'surface_water','Niederschlagsabwasser','eaux_de_surface', 'acque_piovane', 'apa_meteorica', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.infiltration_installation_watertightness ADD CONSTRAINT pkey_qgep_vl_infiltration_installation_watertightness_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.wwtp_structure ADD COLUMN fk_waste_water_treatment_plant varchar(16);
ALTER TABLE qgep_od.wwtp_structure ADD CONSTRAINT rel_wwtp_structure_waste_water_treatment_plant FOREIGN KEY (fk_waste_water_treatment_plant) REFERENCES qgep_od.waste_water_treatment_plant(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.wwtp_structure_kind ADD CONSTRAINT pkey_qgep_vl_wwtp_structure_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (331,331,'sedimentation_basin','Absetzbecken','bassin_de_decantation', 'zzz_Absetzbecken', 'rrr_Absetzbecken', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2974,2974,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (327,327,'aeration_tank','Belebtschlammbecken','bassin_a_boues_activees', 'zzz_Belebtschlammbecken', 'rrr_Belebtschlammbecken', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (329,329,'fixed_bed_reactor','Festbettreaktor','reacteur_a_biomasse_fixee', 'zzz_Festbettreaktor', 'rrr_Festbettreaktor', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (330,330,'submerged_trickling_filter','Tauchtropfkoerper','disque_bacterien_immerge', 'zzz_Tauchtropfkoerper', 'rrr_Tauchtropfkoerper', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (328,328,'trickling_filter','Tropfkoerper','lit_bacterien', 'zzz_Tropfkoerper', 'rrr_Tropfkoerper', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3032,3032,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wwtp_structure_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (326,326,'primary_clarifier','Vorklaerbecken','decanteurs_primaires', 'zzz_Vorklaerbecken', 'rrr_Vorklaerbecken', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.maintenance_event ADD COLUMN fk_measure varchar(16);
ALTER TABLE qgep_od.maintenance_event ADD CONSTRAINT rel_maintenance_event_measure FOREIGN KEY (fk_measure) REFERENCES qgep_od.measure(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.maintenance_event_kind ADD CONSTRAINT pkey_qgep_vl_maintenance_event_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2982,2982,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (28,28,'cleaning','Reinigung','nettoyage', 'pulizia', 'rrr_Reinigung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8830,8830,'restoration_replacement','Sanierung_Erneuerung','rehabilitation_renouvellement', 'risanamento_sostituzione', 'rrr_Sanierung_Erneuerung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8831,8831,'restoration_renovation','Sanierung_Renovierung','rehabilitation_renovation', 'risanamento_rinnovamento', 'rrr_Sanierung_Renovierung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8832,8832,'restoration_repair','Sanierung_Reparatur','rehabilitation_reparation', 'risanamento_riparazione', 'rrr_Sanierung_Reparatur', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8829,8829,'restoration_unknown','Sanierung_unbekannt','rehabilitation_inconnue', 'risanamento_sconosciuto', 'rrr_Sanierung_unbekannt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3045,3045,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9277,9277,'examination','Untersuchung','examen', 'ispezione', 'rrr_Untersuchung', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.maintenance_event_status ADD CONSTRAINT pkey_qgep_vl_maintenance_event_status_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.maintenance_event_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2550,2550,'accomplished','ausgefuehrt','execute', 'eseguito', 'rrr_ausgefuehrt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2549,2549,'planned','geplant','prevu', 'previsto', 'rrr_geplant', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3678,3678,'not_possible','nicht_moeglich','impossible', 'non_possibile', 'rrr_nicht_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.maintenance_event_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3047,3047,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.infiltration_zone_infiltration_capacity ADD CONSTRAINT pkey_qgep_vl_infiltration_zone_infiltration_capacity_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.infiltration_zone_infiltration_capacity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (371,371,'good','gut','bonnes', 'zzz_gut', 'rrr_gut', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.infiltration_zone_infiltration_capacity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (372,372,'moderate','maessig','moyennes', 'zzz_maessig', 'rrr_maessig', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_zone_infiltration_capacity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (373,373,'bad','schlecht','mauvaises', 'zzz_schlecht', 'rrr_schlecht', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_zone_infiltration_capacity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3073,3073,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.infiltration_zone_infiltration_capacity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2996,2996,'not_allowed','unzulaessig','non_admis', 'zzz_unzulaessig', 'rrr_unzulaessig', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.drainage_system_kind ADD CONSTRAINT pkey_qgep_vl_drainage_system_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.drainage_system_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4783,4783,'amelioration','Melioration','melioration', 'zzz_melizzazione', 'rrr_Melioration', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainage_system_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2722,2722,'mixed_system','Mischsystem','systeme_unitaire', 'sistema_misto', 'rrr_Mischsystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainage_system_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2724,2724,'modified_system','ModifiziertesSystem','systeme_modifie', 'sistema_modificato', 'rrr_ModifiziertesSystem', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.drainage_system_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2723,2723,'separated_system','Trennsystem','systeme_separatif', 'sistema_separato', 'rrr_Trennsystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainage_system_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3060,3060,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.waste_water_treatment_kind ADD CONSTRAINT pkey_qgep_vl_waste_water_treatment_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3210,3210,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (387,387,'biological','biologisch','biologique', 'zzz_biologisch', 'rrr_biologisch', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (388,388,'chemical','chemisch','chimique', 'zzz_chemisch', 'rrr_chemisch', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (389,389,'filtration','Filtration','filtration', 'zzz_Filtration', 'rrr_Filtration', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (366,366,'mechanical','mechanisch','mecanique', 'zzz_mechanisch', 'rrr_mechanisch', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.waste_water_treatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3076,3076,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.sludge_treatment_stabilisation ADD CONSTRAINT pkey_qgep_vl_sludge_treatment_stabilisation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (141,141,'aerob_cold','aerobkalt','aerobie_froid', 'zzz_aerobkalt', 'rrr_aerobkalt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (332,332,'aerobthermophil','aerobthermophil','aerobie_thermophile', 'zzz_aerobthermophil', 'rrr_aerobthermophil', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (333,333,'anaerob_cold','anaerobkalt','anaerobie_froid', 'zzz_anaerobkalt', 'rrr_anaerobkalt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (334,334,'anaerob_mesophil','anaerobmesophil','anaerobie_mesophile', 'zzz_anaerobmesophil', 'rrr_anaerobmesophil', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (335,335,'anaerob_thermophil','anaerobthermophil','anaerobie_thermophile', 'zzz_anaerobthermophil', 'rrr_anaerobthermophil', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2994,2994,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.sludge_treatment_stabilisation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3004,3004,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.reach_point_outlet_shape ADD CONSTRAINT pkey_qgep_vl_reach_point_outlet_shape_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.reach_point_pipe_closure () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.reach_point_pipe_closure ADD CONSTRAINT pkey_qgep_vl_reach_point_pipe_closure_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_point_pipe_closure (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8999,8999,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_point_pipe_closure (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9000,9000,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_point_pipe_closure (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9001,9001,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.reach_point ADD CONSTRAINT fkey_vl_reach_point_pipe_closure FOREIGN KEY (pipe_closure)
 REFERENCES qgep_vl.reach_point_pipe_closure (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
CREATE TABLE qgep_vl.wastewater_node_elevation_accuray () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.wastewater_node_elevation_accuray ADD CONSTRAINT pkey_qgep_vl_wastewater_node_elevation_accuray_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.wastewater_node_elevation_accuray (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9061,9061,'more_than_6cm','groesser_6cm','plusque_6cm', 'piu_6cm', 'mai_mare_6cm', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_elevation_accuray (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9062,9062,'plusminus_1cm','plusminus_1cm','plus_moins_1cm', 'piu_meno_1cm', 'plus_minus_1cm', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_elevation_accuray (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9063,9063,'plusminus_3cm','plusminus_3cm','plus_moins_3cm', 'piu_meno_3cm', 'plus_minus_3cm', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_elevation_accuray (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9064,9064,'plusminus_6cm','plusminus_6cm','plus_moins_6cm', 'piu_meno_6cm', 'plus_minus_6cm', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_elevation_accuray (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9060,9060,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.wastewater_node ADD CONSTRAINT fkey_vl_wastewater_node_elevation_accuray FOREIGN KEY (elevation_accuray)
 REFERENCES qgep_vl.wastewater_node_elevation_accuray (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.wastewater_node_function_node_amelioration () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.wastewater_node_function_node_amelioration ADD CONSTRAINT pkey_qgep_vl_wastewater_node_function_node_amelioration_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.wastewater_node_function_node_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4621,4621,'slope_change','Gefaellsbruch','changement_de_pente', 'zzz_Gefaellsbruch', 'rrr_Gefaellsbruch', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_function_node_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9083,9083,'slope_caliber_change','Gefaellsbruch_Kaliberwechsel','changement_de_pente_et_calibre', 'zzz_Gefaellsbruch_Kaliberwechsel', 'rrr_Gefaellsbruch_Kaliberwechsel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_function_node_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4623,4623,'change_of_diameter','Kaliberwechsel','changement_de_calibre', 'zzz_Kaliberwechsel', 'rr_Kaliberwechsel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_node_function_node_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4622,4622,'standard_manhole','Normschacht','chambre_standard', 'zzz_Normschacht', 'rrr_Normschacht', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.wastewater_node_function_node_amelioration (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4620,4620,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.wastewater_node ADD CONSTRAINT fkey_vl_wastewater_node_function_node_amelioration FOREIGN KEY (function_node_amelioration)
 REFERENCES qgep_vl.wastewater_node_function_node_amelioration (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.reach_inside_coating ADD CONSTRAINT pkey_qgep_vl_reach_inside_coating_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.reach_leak_protection () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.reach_leak_protection ADD CONSTRAINT pkey_qgep_vl_reach_leak_protection_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_leak_protection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8710,8710,'inexistent','nicht_vorhanden','inexistant', 'non_presente', 'inexistent', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_leak_protection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8711,8711,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_leak_protection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8709,8709,'existent','vorhanden','existant', 'presente', 'existent', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.reach ADD CONSTRAINT fkey_vl_reach_leak_protection FOREIGN KEY (leak_protection)
 REFERENCES qgep_vl.reach_leak_protection (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_vl.reach_material ADD CONSTRAINT pkey_qgep_vl_reach_material_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.reach_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5078,5078,'plastic_polyester_GUP','Kunststoff_Polyester_GUP','matiere_synthetique_polyester_PRV', 'materiale_sintetico_poliestere_GUP', 'plastic_poliester_GUP', 'GUP', 'GUP', 'GUP', 'GUP', '', 'true');
...
ALTER TABLE qgep_vl.reach_reliner_material ADD CONSTRAINT pkey_qgep_vl_reach_reliner_material_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_reliner_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6459,6459,'other','andere','autre', 'altro', 'altul', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.reach_reliner_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6467,6467,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscut', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.reach_relining_construction ADD CONSTRAINT pkey_qgep_vl_reach_relining_construction_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_relining_construction (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6448,6448,'other','andere','autre', 'altro', 'alta', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.reach_relining_construction (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6453,6453,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.reach_relining_kind ADD CONSTRAINT pkey_qgep_vl_reach_relining_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_relining_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6455,6455,'full_reach','ganze_Haltung','troncon_entier', 'condotta_intera', 'tronson_intreg', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_relining_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6456,6456,'partial','partiell','partiellement', 'parziale', 'partial', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.mechanical_pretreatment ADD CONSTRAINT rel_mechanical_pretreatment_wastewater_structure FOREIGN KEY (fk_wastewater_structure) REFERENCES qgep_od.wastewater_structure(obj_id) ON UPDATE CASCADE ON DELETE cascade;
...
ALTER TABLE qgep_vl.mechanical_pretreatment_kind ADD CONSTRAINT pkey_qgep_vl_mechanical_pretreatment_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.mechanical_pretreatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3317,3317,'filter_bag','Filtersack','percolateur', 'zzz_Filtersack', 'rrr_Filtersack', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.mechanical_pretreatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3319,3319,'artificial_adsorber','KuenstlicherAdsorber','adsorbeur_artificiel', 'zzz_KuenstlicherAdsorber', 'rrr_KuenstlicherAdsorber', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.mechanical_pretreatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3318,3318,'swale_french_drain_system','MuldenRigolenSystem','systeme_cuvettes_rigoles', 'zzz_MuldenRigolenSystem', 'rrr_MuldenRigolenSystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.mechanical_pretreatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3320,3320,'slurry_collector','Schlammsammler','collecteur_de_boue', 'pozzetto_decantazione', 'colector_aluviuni', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.mechanical_pretreatment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3322,3322,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.retention_body_kind ADD CONSTRAINT pkey_qgep_vl_retention_body_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2992,2992,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (346,346,'retention_in_habitat','Biotop','retention_dans_bassins_et_depressions', 'zzz_Biotop', 'rrr_Biotop', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (345,345,'roof_retention','Dachretention','retention_sur_toits', 'zzz_Dachretention', 'rrr_Dachretention', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (348,348,'parking_lot','Parkplatz','retention_sur_routes_et_places', 'zzz_Parkplatz', 'rrr_Parkplaetze', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (347,347,'accumulation_channel','Staukanal','retention_dans_canalisations_stockage', 'zzz_Staukanal', 'rrr_Staukanal', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.retention_body_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3031,3031,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_char_kind_overflow_char ADD CONSTRAINT pkey_qgep_vl_overflow_char_kind_overflow_char_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.overflow_char_kind_overflow_char (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6221,6221,'qq','QQ','QQ', 'QQ', 'QQ', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_char_kind_overflow_char (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6228,6228,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.dryweather_flume_material ADD CONSTRAINT pkey_qgep_vl_dryweather_flume_material_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.dryweather_flume_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3221,3221,'other','andere','autres', 'altro', 'alta', 'O', 'A', 'AU', '', '', 'true');
...
ALTER TABLE qgep_vl.cover_material ADD CONSTRAINT pkey_qgep_vl_cover_material_code PRIMARY KEY (code);
 ...
 INSERT INTO qgep_vl.cover_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (234,234,'concrete','Beton','beton', 'calcestruzzo', 'beton', 'C', 'B', 'B', '', '', 'true');
 INSERT INTO qgep_vl.cover_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (233,233,'cast_iron','Guss','fonte', 'ghisa', 'fonta', '', 'G', 'F', '', '', 'true');
 INSERT INTO qgep_vl.cover_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5547,5547,'cast_iron_with_pavement_filling','Guss_mit_Belagsfuellung','fonte_avec_remplissage_enrobe', 'zzz_Guss_mit_Belagsfuellung', 'fonta_cu_umplutura', 'CIP', 'GBL', 'FRE', '', '', 'true');
 INSERT INTO qgep_vl.cover_material (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (235,235,'cast_iron_with_concrete_filling','Guss_mit_Betonfuellung','fonte_avec_remplissage_beton', 'zzz_Guss_mit_Betonfuellung', 'fonta_cu_umplutura_beton', '', 'GBT', 'FRB', '', '', 'true');
...
ALTER TABLE qgep_vl.cover_sludge_bucket ADD CONSTRAINT pkey_qgep_vl_cover_sludge_bucket_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.cover_sludge_bucket (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (423,423,'inexistent','nicht_vorhanden','inexistant', 'non_presente', 'inexistent', '', 'NV', 'IE', '', '', 'true');
 ...
 INSERT INTO qgep_vl.cover_sludge_bucket (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (422,422,'existent','vorhanden','existant', 'presente', 'existent', '', 'V', 'E', '', '', 'true');
...
ALTER TABLE qgep_vl.electric_equipment_kind ADD CONSTRAINT pkey_qgep_vl_electric_equipment_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2980,2980,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (376,376,'illumination','Beleuchtung','eclairage', 'zzz_Beleuchtung', 'rrr_Beleuchtung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3255,3255,'remote_control_system','Fernwirkanlage','installation_de_telecommande', 'zzz_Fernwirkanlage', 'rrr_Fernwirkanlage', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (378,378,'radio_unit','Funk','radio', 'zzz_Funk', 'rrr_Funk', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (377,377,'phone','Telephon','telephone', 'telefono', 'telefon', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electric_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3038,3038,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.electromechanical_equipment_kind ADD CONSTRAINT pkey_qgep_vl_electromechanical_equipment_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.electromechanical_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2981,2981,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electromechanical_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (380,380,'leakage_water_pump','Leckwasserpumpe','pompe_d_epuisement', 'zzz_Leckwasserpumpe', 'rrr_Leckwasserpumpe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electromechanical_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (337,337,'air_dehumidifier','Luftentfeuchter','deshumidificateur', 'zzz_Luftentfeuchter', 'rrr_Luftentfeuchter', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.electromechanical_equipment_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3072,3072,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.benching_kind ADD CONSTRAINT pkey_qgep_vl_benching_kind_code PRIMARY KEY (code);
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.flushing_nozzle ADD CONSTRAINT oorel_od_flushing_nozzle_structure_part FOREIGN KEY (obj_id) REFERENCES qgep_od.structure_part(obj_id) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE qgep_vl.individual_surface_function ADD CONSTRAINT pkey_qgep_vl_individual_surface_function_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.individual_surface_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2979,2979,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3466,3466,'railway_site','Bahnanlagen','installation_ferroviaire', 'zzz_Bahnanlagen', 'rrr_Bahnanlagen', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.individual_surface_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3467,3467,'parking_lot','Parkplaetze','parkings', 'zzz_Parkplaetze', 'rrr_Parkplaetze', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.individual_surface_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3029,3029,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.individual_surface_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3463,3463,'forecourt_and_access_road','VorplaetzeZufahrten','places_devant_entree_acces', 'zzz_VorplaetzeZufahrten', 'rrr_VorplaetzeZufahrten', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.individual_surface_pavement ADD CONSTRAINT pkey_qgep_vl_individual_surface_pavement_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2978,2978,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2031,2031,'paved','befestigt','impermeabilise', 'zzz_befestigt', 'rrr_befestigt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2032,2032,'forested','bestockt','boise', 'zzz_bestockt', 'rrr_bestockt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2033,2033,'soil_covered','humusiert','couverture_vegetale', 'zzz_humusiert', 'rrr_humusiert', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3030,3030,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.individual_surface_pavement (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2034,2034,'barren','vegetationslos','sans_vegetation', 'zzz_vegetationslos', 'rrr_vegetationslos', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.log_card ADD COLUMN fk_pwwf_wastewater_node varchar(16);
ALTER TABLE qgep_od.log_card ADD CONSTRAINT rel_log_card_pwwf_wastewater_node FOREIGN KEY (fk_pwwf_wastewater_node) REFERENCES qgep_od.wastewater_node(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.log_card ADD COLUMN fk_agency varchar(16);
ALTER TABLE qgep_od.log_card ADD CONSTRAINT rel_log_card_agency FOREIGN KEY (fk_agency) REFERENCES qgep_od.organisation(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.log_card ADD COLUMN fk_location_municipality varchar(16);
ALTER TABLE qgep_od.log_card ADD CONSTRAINT rel_log_card_location_municipality FOREIGN KEY (fk_location_municipality) REFERENCES qgep_od.organisation(obj_id) ON UPDATE CASCADE ON DELETE set null;
CREATE TABLE qgep_vl.log_card_control_remote_control () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.log_card_control_remote_control ADD CONSTRAINT pkey_qgep_vl_log_card_control_remote_control_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8532,8532,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8527,8527,'no_control','keine_Steuerung','aucune_commande', 'nessun_comando', 'rrr_keine_Steuerung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8528,8528,'local_control','lokale_Steuerung','commande_locale', 'comando_locale', 'rrr_lokale_Steuerung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8529,8529,'transmission_alarm','Uebermittlung_Alarm','transmission_alarme', 'trasmissione_allarme', 'rrr_Uebermittlung_Alarm', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8530,8530,'transmission_measuring_signals','Uebermittlung_Messsignale','transmission_signaux_mesure', 'trasmissione_segnali_misura', 'rrr_Uebermittlung_Messsignale', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8533,8533,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_control_remote_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8531,8531,'interconnection_control','Verbundsteuerung','commande_combinee', 'comando_composito', 'rrr_Verbundsteuerung', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.log_card ADD CONSTRAINT fkey_vl_log_card_control_remote_control FOREIGN KEY (control_remote_control)
 REFERENCES qgep_vl.log_card_control_remote_control (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.log_card_information_source () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.log_card_information_source ADD CONSTRAINT pkey_qgep_vl_log_card_information_source_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.log_card_information_source (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5601,5601,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_information_source (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5604,5604,'gwdp_wwtp_catchment_area','GEP_ARA_Einzugsgebiet','bassin_versant_STEP', 'PGS_IDA_bacino_imbrifero', 'rrr_GEP_ARA_Einzugsgebiet', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_information_source (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5602,5602,'gwdp_responsible_body','GEP_Traegerschaft','entite_responsable_PGEE', 'PGS_ente_responsabile', 'rrr_GEP_Traegerschaft', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.log_card_information_source (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5603,5603,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.log_card ADD CONSTRAINT fkey_vl_log_card_information_source FOREIGN KEY (information_source)
 REFERENCES qgep_vl.log_card_information_source (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_od.catchment_area ADD COLUMN fk_special_building_rw_planned varchar(16);
ALTER TABLE qgep_od.catchment_area ADD CONSTRAINT rel_catchment_area_special_building_rw_planned FOREIGN KEY (fk_special_building_rw_planned) REFERENCES qgep_od.log_card(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.catchment_area ADD COLUMN fk_special_building_rw_current varchar(16);
ALTER TABLE qgep_od.catchment_area ADD CONSTRAINT rel_catchment_area_special_building_rw_current FOREIGN KEY (fk_special_building_rw_current) REFERENCES qgep_od.log_card(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.catchment_area ADD COLUMN fk_special_building_ww_planned varchar(16);
ALTER TABLE qgep_od.catchment_area ADD CONSTRAINT rel_catchment_area_special_building_ww_planned FOREIGN KEY (fk_special_building_ww_planned) REFERENCES qgep_od.log_card(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.catchment_area ADD COLUMN fk_special_building_ww_current varchar(16);
ALTER TABLE qgep_od.catchment_area ADD CONSTRAINT rel_catchment_area_special_building_ww_current FOREIGN KEY (fk_special_building_ww_current) REFERENCES qgep_od.log_card(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.catchment_area_direct_discharge_current ADD CONSTRAINT pkey_qgep_vl_catchment_area_direct_discharge_current_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_direct_discharge_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5457,5457,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_direct_discharge_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5458,5458,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_direct_discharge_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5463,5463,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_direct_discharge_planned ADD CONSTRAINT pkey_qgep_vl_catchment_area_direct_discharge_planned_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_direct_discharge_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5459,5459,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_direct_discharge_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5460,5460,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_direct_discharge_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5464,5464,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_drainage_system_current ADD CONSTRAINT pkey_qgep_vl_catchment_area_drainage_system_current_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_drainage_system_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9068,9068,'drainage_system','Drainagesystem','systeme_de_drainage', 'sistema_di_drenaggio', 'rrr_Drainagesystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5186,5186,'mixed_system','Mischsystem','systeme_unitaire', 'sistema_misto', 'rrr_Mischsystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5188,5188,'modified_system','ModifiziertesSystem','systeme_modifie', 'sistema_modificato', 'rrr_ModifiziertesSystem', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.catchment_area_drainage_system_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5537,5537,'not_drained','nicht_entwaessert','non_evacue', 'non_evacuato', 'rrr_nicht_entwaessert', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.catchment_area_drainage_system_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5189,5189,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_drainage_system_planned ADD CONSTRAINT pkey_qgep_vl_catchment_area_drainage_system_planned_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9067,9067,'drainage_system','Drainagesystem','systeme_de_drainage', 'sistema_di_drenaggio', 'rrr_Drainagesystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5191,5191,'mixed_system','Mischsystem','systeme_unitaire', 'sistema_misto', 'rrr_Mischsystem', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5536,5536,'not_drained','nicht_entwaessert','non_evacue', 'non_evacuato', 'rrr_nicht_entwaessert', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5192,5192,'separated_system','Trennsystem','systeme_separatif', 'sistema_separato', 'rrr_Trennsystem', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5195,5195,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_drainage_system_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8692,8692,'separated_system_in_preparation','vorbereitetes_Trennsystem','systeme_separatif_prepare', 'predisposizione_sistema_separato', 'rrr_vorbereitetes_Trennsystem', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_infiltration_current ADD CONSTRAINT pkey_qgep_vl_catchment_area_infiltration_current_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_infiltration_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5452,5452,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_infiltration_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5453,5453,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_infiltration_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5165,5165,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_infiltration_planned ADD CONSTRAINT pkey_qgep_vl_catchment_area_infiltration_planned_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_infiltration_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5461,5461,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_infiltration_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5462,5462,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_infiltration_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5170,5170,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_retention_current ADD CONSTRAINT pkey_qgep_vl_catchment_area_retention_current_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_retention_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5467,5467,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_retention_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5468,5468,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_retention_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5469,5469,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_retention_planned ADD CONSTRAINT pkey_qgep_vl_catchment_area_retention_planned_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_retention_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5470,5470,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_retention_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5471,5471,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_retention_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5472,5472,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.measuring_point_damming_device ADD CONSTRAINT pkey_qgep_vl_measuring_point_damming_device_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measuring_point_damming_device (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5720,5720,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_damming_device (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5721,5721,'none','keiner','aucun', 'nessuno', 'inexistent', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_damming_device (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5722,5722,'overflow_weir','Ueberfallwehr','lame_deversante', 'sfioratore', 'rrr_Ueberfallwehr', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_damming_device (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5724,5724,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_damming_device (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5723,5723,'venturi_necking','Venturieinschnuerung','etranglement_venturi', 'restringimento_venturi', 'rrr_Venturieinschnuerung', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.measuring_point_purpose ADD CONSTRAINT pkey_qgep_vl_measuring_point_purpose_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measuring_point_purpose (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4595,4595,'both','beides','les_deux', 'entrambi', 'rrr_beides', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_purpose (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4593,4593,'cost_sharing','Kostenverteilung','repartition_des_couts', 'ripartizione_costi', 'rrr_Kostenverteilung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_purpose (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4594,4594,'technical_purpose','technischer_Zweck','but_technique', 'scopo_tecnico', 'rrr_technischer_Zweck', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_point_purpose (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4592,4592,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.measuring_device_kind ADD CONSTRAINT pkey_qgep_vl_measuring_device_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5702,5702,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5703,5703,'static_sounding_stick','Drucksonde','sonde_de_pression', 'sensore_pressione', 'rrr_Drucksonde', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5704,5704,'bubbler_system','Lufteinperlung','injection_bulles_d_air', 'insufflazione', 'rrr_Lufteinperlung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5705,5705,'EMF_partly_filled','MID_teilgefuellt','MID_partiellement_rempli', 'MPE_parzialmente_riempito', 'rrr_MID_teilgefuellt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5706,5706,'EMF_filled','MID_vollgefuellt','MID_rempli', 'MPE_riempito', 'rrr_MID_vollgefuellt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5707,5707,'radar','Radar','radar', 'radar', 'radar', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5708,5708,'float','Schwimmer','flotteur', 'galleggiante', 'rrr_Schwimmer', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6322,6322,'ultrasound','Ultraschall','ultrason', 'ultrasuono', 'ultrasunete', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measuring_device_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5709,5709,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.measurement_series ADD COLUMN fk_wastewater_networkelement varchar(16);
ALTER TABLE qgep_od.measurement_series ADD CONSTRAINT rel_measurement_series_wastewater_networkelement FOREIGN KEY (fk_wastewater_networkelement) REFERENCES qgep_od.wastewater_networkelement(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.measurement_series_kind ADD CONSTRAINT pkey_qgep_vl_measurement_series_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measurement_series_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3217,3217,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measurement_series_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2646,2646,'continuous','kontinuierlich','continu', 'zzz_kontinuierlich', 'rrr_kontinuierlich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measurement_series_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2647,2647,'rain_weather','Regenwetter','temps_de_pluie', 'tempo_pioggia', 'rrr_Regenwetter', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measurement_series_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8778,8778,'simulation','Simulation','simulation', 'simulazione', 'rrr_Simulation', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measurement_series_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3053,3053,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.measurement_result_measurement_type ADD CONSTRAINT pkey_qgep_vl_measurement_result_measurement_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.measurement_result_measurement_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5732,5732,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.measurement_result_measurement_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5733,5733,'flow','Durchfluss','debit', 'portata', 'rrr_Durchfluss', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.measurement_result_measurement_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5735,5735,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_actuation ADD CONSTRAINT pkey_qgep_vl_overflow_actuation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.overflow_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3667,3667,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.overflow_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (300,300,'none','keiner','aucun', 'nessuno', 'inexistent', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.overflow_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3005,3005,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_adjustability ADD CONSTRAINT pkey_qgep_vl_overflow_adjustability_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.overflow_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (355,355,'fixed','fest','fixe', 'zzz_fest', 'rrr_fix', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3021,3021,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (356,356,'adjustable','verstellbar','reglable', 'zzz_verstellbar', 'rrr_verstellbar', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_control ADD CONSTRAINT pkey_qgep_vl_overflow_control_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.overflow_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (308,308,'closed_loop_control','geregelt','avec_regulation', 'zzz_geregelt', 'rrr_geregelt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (307,307,'open_loop_control','gesteuert','avec_commande', 'zzz_gesteuert', 'rrr_gesteuert', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.overflow_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3028,3028,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_function ADD CONSTRAINT pkey_qgep_vl_overflow_function_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3228,3228,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3384,3384,'internal','intern','interne', 'zzz_intern', 'rrr_intern', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (217,217,'emergency_overflow','Notentlastung','deversoir_de_secours', 'scarico_emergenza', 'rrr_Notentlastung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5544,5544,'stormwater_overflow','Regenueberlauf','deversoir_d_orage', 'scaricatore_piena', 'preaplin', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5546,5546,'internal_overflow','Trennueberlauf','deversoir_interne', 'zzz_Trennueberlauf', 'rrr_Trennueberlauf', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3010,3010,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.overflow_signal_transmission ADD CONSTRAINT pkey_qgep_vl_overflow_signal_transmission_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.overflow_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2694,2694,'receiving','empfangen','recevoir', 'zzz_empfangen', 'rrr_empfangen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2693,2693,'sending','senden','emettre', 'zzz_senden', 'rrr_senden', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2695,2695,'sending_receiving','senden_empfangen','emettre_recevoir', 'zzz_senden_empfangen', 'rrr_senden_empfangen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.overflow_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3056,3056,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.throttle_shut_off_unit_actuation ADD CONSTRAINT pkey_qgep_vl_throttle_shut_off_unit_actuation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.throttle_shut_off_unit_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3213,3213,'other','andere','autres', 'altro', 'altul', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.throttle_shut_off_unit_adjustability ADD CONSTRAINT pkey_qgep_vl_throttle_shut_off_unit_adjustability_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.throttle_shut_off_unit_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3159,3159,'fixed','fest','fixe', 'zzz_fest', 'rrr_fix', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3161,3161,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_adjustability (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3160,3160,'adjustable','verstellbar','reglable', 'zzz_verstellbar', 'rrr_verstellbar', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.throttle_shut_off_unit_control ADD CONSTRAINT pkey_qgep_vl_throttle_shut_off_unit_control_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.throttle_shut_off_unit_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3162,3162,'closed_loop_control','geregelt','avec_regulation', 'zzz_geregelt', 'rrr_geregelt', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3163,3163,'open_loop_control','gesteuert','avec_commande', 'zzz_gesteuert', 'rrr_gesteuert', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.throttle_shut_off_unit_control (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3164,3164,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.throttle_shut_off_unit_kind ADD CONSTRAINT pkey_qgep_vl_throttle_shut_off_unit_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2973,2973,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2691,2691,'stop_log','Dammbalken','batardeau', 'zzz_Dammbalken', 'rrr_Dammbalken', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (252,252,'throttle_flap','Drosselklappe','clapet_de_limitation', 'zzz_Drosselklappe', 'rrr_Drosselklappe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (135,135,'throttle_valve','Drosselschieber','vanne_de_limitation', 'zzz_Drosselschieber', 'rrr_Drosselschieber', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6490,6490,'throttle_section','Drosselstrecke','conduite_d_etranglement', 'tratta_regolazione', 'rrr_Drosselstrecke', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6491,6491,'leapingweir','Leapingwehr','leaping_weir', 'leaping_weir', 'rrr_Leapingwehr', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6492,6492,'pomp','Pumpe','pompe', 'pompa', 'rrr_Pumpe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2690,2690,'backflow_flap','Rueckstauklappe','clapet_de_non_retour_a_battant', 'clappa_anti_rigurgito', 'clapeta _antirefulare', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2688,2688,'valve','Schieber','vanne', 'saracinesca', 'rrr_Schieber', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (134,134,'tube_throttle','Schlauchdrossel','limiteur_a_membrane', 'zzz_Schlauchdrossel', 'rrr_Schlauchdrossel', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5755,5755,'gate_shield','Stauschild','plaque_de_retenue', 'paratoia_cilindrica', 'rrr_Stauschild', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3046,3046,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (133,133,'whirl_throttle','Wirbeldrossel','limiteur_a_vortex', 'zzz_Wirbeldrossel', 'rrr_Wirbeldrossel', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.throttle_shut_off_unit_signal_transmission ADD CONSTRAINT pkey_qgep_vl_throttle_shut_off_unit_signal_transmission_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.throttle_shut_off_unit_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3171,3171,'receiving','empfangen','recevoir', 'zzz_empfangen', 'rrr_empfangen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3172,3172,'sending','senden','emettre', 'zzz_senden', 'rrr_senden', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3169,3169,'sending_receiving','senden_empfangen','emettre_recevoir', 'zzz_senden_empfangen', 'rrr_senden_empfangen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.throttle_shut_off_unit_signal_transmission (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3170,3170,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.prank_weir_weir_edge ADD CONSTRAINT pkey_qgep_vl_prank_weir_weir_edge_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.prank_weir_weir_edge (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2995,2995,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.prank_weir_weir_edge (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (351,351,'rectangular','rechteckig','angulaire', 'zzz_rechteckig', 'rrr_rechteckig', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.prank_weir_weir_edge (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3014,3014,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.prank_weir_weir_kind ADD CONSTRAINT pkey_qgep_vl_prank_weir_weir_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.prank_weir_weir_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5772,5772,'raised','hochgezogen','a_seuil_sureleve', 'laterale_alto', 'rrr_hochgezogen', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.pump_construction_type ADD CONSTRAINT pkey_qgep_vl_pump_construction_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2983,2983,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2662,2662,'compressed_air_system','Druckluftanlage','systeme_a_air_comprime', 'impianto_aria_compressa', 'rrr_Druckluftanlage', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (309,309,'centrifugal_pump','Kreiselpumpe','pompe_centrifuge', 'pompa_centrifuga', 'rrr_Kreiselpumpe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (310,310,'screw_pump','Schneckenpumpe','pompe_a_vis', 'pompa_a_vite', 'rrr_Schneckenpumpe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3082,3082,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_construction_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (2661,2661,'vacuum_system','Vakuumanlage','systeme_a_vide_d_air', 'impinato_a_vuoto_aria', 'rrr_Vakuumanlage', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.pump_placement_of_actuation ADD CONSTRAINT pkey_qgep_vl_pump_placement_of_actuation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.pump_placement_of_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (318,318,'wet','nass','immerge', 'zzz_nass', 'rrr_nass', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_placement_of_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (311,311,'dry','trocken','non_submersible', 'zzz_trocken', 'rrr_trocken', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_placement_of_actuation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3070,3070,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.pump_placement_of_pump ADD CONSTRAINT pkey_qgep_vl_pump_placement_of_pump_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.pump_placement_of_pump (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (362,362,'horizontal','horizontal','horizontal', 'zzz_horizontal', 'rrr_horizontal', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_placement_of_pump (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3071,3071,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.pump_placement_of_pump (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (363,363,'vertical','vertikal','vertical', 'zzz_vertikal', 'rrr_vertikal', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.leapingweir_opening_shape ADD CONSTRAINT pkey_qgep_vl_leapingweir_opening_shape_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.leapingweir_opening_shape (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3581,3581,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.leapingweir_opening_shape (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3582,3582,'circle','Kreis','circulaire', 'zzz_Kreis', 'rrr_Kreis', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.leapingweir_opening_shape (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3585,3585,'parable','Parabel','parabolique', 'zzz_Parabel', 'rrr_Parabel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.leapingweir_opening_shape (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3583,3583,'rectangular','Rechteck','rectangulaire', 'zzz_Rechteck', 'rrr_Rechteck', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.leapingweir_opening_shape (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (3584,3584,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.maintenance ADD CONSTRAINT oorel_od_maintenance_maintenance_event FOREIGN KEY (obj_id) REFERENCES qgep_od.maintenance_event(obj_id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT oorel_od_bio_ecol_assessment_maintenance_event FOREIGN KEY (obj_id) REFERENCES qgep_od.maintenance_event(obj_id) ON DELETE CASCADE ON UPDATE CASCADE;
CREATE TABLE qgep_vl.bio_ecol_assessment_comparison_last () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_comparison_last ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_comparison_last_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5896,5896,'equal','gleich','egal', 'uguale', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6328,6328,'no_comparison_possible','kein_Vergleich_moeglich','aucune_comparaison_possible', 'confronto_non_possibile', 'rrr_kein_Vergleich_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8572,8572,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6271,6271,'unclear','unklar','pas_clair', 'non_chiaro', 'rrr_unklar', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6269,6269,'improvement','Verbesserung','amelioration', 'miglioramento', 'rrrr_Verbesserung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_comparison_last (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6270,6270,'worsening','Verschlechterung','deterioration', 'peggioramento', 'rrr_Verschlechterung', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_comparison_last FOREIGN KEY (comparison_last)
 REFERENCES qgep_vl.bio_ecol_assessment_comparison_last (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_impact_auxiliary_indic () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_impact_auxiliary_indic ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_impact_auxiliary_indic_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8687,8687,'big','gross','elevee', 'grande', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8685,8685,'none_or_small','kein_klein','aucune_petite', 'nessuno_piccolo', 'rrr_kein_klein', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8689,8689,'no_conclusion_possible','keine_Aussage_moeglich','aucun_avis_possible', 'giudizio_non_possibile', 'rrr_keine_Aussage_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8686,8686,'moderate','mittel','moyen', 'medio', 'rrr_mittel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8690,8690,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8688,8688,'unclear','unklar','pas_clair', 'non_chiaro', 'rrr_unklar', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_impact_auxiliary_indic FOREIGN KEY (impact_auxiliary_indic)
 REFERENCES qgep_vl.bio_ecol_assessment_impact_auxiliary_indic (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_impact_external_aspect () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_impact_external_aspect ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_impact_external_aspect_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8666,8666,'big','gross','elevee', 'grande', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8664,8664,'none','kein','aucun', 'nessuno', 'niciun', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8668,8668,'no_conclusion_possible','keine_Aussage_moeglich','aucun_avis_possible', 'giudizio_non_possibile', 'rrr_keine_Aussage_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8665,8665,'small_medium','klein_mittel','faible_moyenne', 'medio_piccolo', 'rrr_klein_mittel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8669,8669,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_external_aspect (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8667,8667,'unclear','unklar','pas_clair', 'non_chiaro', 'rrr_unklar', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_impact_external_aspect FOREIGN KEY (impact_external_aspect)
 REFERENCES qgep_vl.bio_ecol_assessment_impact_external_aspect (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_impact_macroinvertebrates () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_impact_macroinvertebrates ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_impact_macroinvertebrates_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8673,8673,'big','gross','elevee', 'grande', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8671,8671,'none_or_small','kein_klein','aucune_petite', 'nessuno_piccolo', 'rrr_kein_klein', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8675,8675,'no_conclusion_possible','keine_Aussage_moeglich','aucun_avis_possible', 'giudizio_non_possibile', 'rrr_keine_Aussage_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8672,8672,'moderate','mittel','moyen', 'medio', 'rrr_mittel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8676,8676,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8674,8674,'unclear','unklar','pas_clair', 'non_chiaro', 'rrr_unklar', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_impact_macroinvertebrates FOREIGN KEY (impact_macroinvertebrates)
 REFERENCES qgep_vl.bio_ecol_assessment_impact_macroinvertebrates (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_impact_water_plants () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_impact_water_plants ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_impact_water_plants_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8680,8680,'big','gross','elevee', 'grande', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8678,8678,'none_or_small','kein_klein','aucune_petite', 'nessuno_piccolo', 'rrr_kein_klein', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8682,8682,'no_conclusion_possible','keine_Aussage_moeglich','aucun_avis_possible', 'giudizio_non_possibile', 'rrr_keine_Aussage_moeglich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8679,8679,'moderate','mittel','moyen', 'medio', 'rrr_mittel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8683,8683,'unknown','unbekannt','inconnue', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_impact_water_plants (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8681,8681,'unclear','unklar','pas_clair', 'non_chiaro', 'rrr_unklar', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_impact_water_plants FOREIGN KEY (impact_water_plants)
 REFERENCES qgep_vl.bio_ecol_assessment_impact_water_plants (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_intervention_demand () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_intervention_demand ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_intervention_demand_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5945,5945,'yes_short_term','ja_kurzfristig','oui_a_court_terme', 'si_corto_termine', 'rrr_ja_kurzfristig', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6272,6272,'yes_long_term','ja_laengerfristig','oui_a_long_terme', 'si_lungo_termine', 'rrr_ja_laengerfristig', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8695,8695,'no_conclusion_possible','keine_Aussage_moeglich','aucun_avis_possible', 'giudizio_non_possibile', 'nicio_declaratie_posibila', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5944,5944,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9093,9093,'clarify_status','Status_klaeren','clarifier_statut', 'chiarire_stato', 'rrr_Status_klaeren', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_intervention_demand (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5946,5946,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_intervention_demand FOREIGN KEY (intervention_demand)
 REFERENCES qgep_vl.bio_ecol_assessment_intervention_demand (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_io_calculation () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_io_calculation ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_io_calculation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_io_calculation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5953,5953,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_io_calculation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5952,5952,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_io_calculation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5954,5954,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_io_calculation FOREIGN KEY (io_calculation)
 REFERENCES qgep_vl.bio_ecol_assessment_io_calculation (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_relevance_matrix () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_relevance_matrix ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_relevance_matrix_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_relevance_matrix (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5949,5949,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_relevance_matrix (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5948,5948,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_relevance_matrix (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5950,5950,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_relevance_matrix FOREIGN KEY (relevance_matrix)
 REFERENCES qgep_vl.bio_ecol_assessment_relevance_matrix (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.bio_ecol_assessment_type_water_body () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.bio_ecol_assessment_type_water_body ADD CONSTRAINT pkey_qgep_vl_bio_ecol_assessment_type_water_body_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8492,8492,'river_backwater','Fluss_Stau','retention', 'corso_d_acqua_di_accummulo', 'rrr_Fluss_Stau', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5884,5884,'large_river','Groesseres_Fliessgewaesser','gros_cours_d_eau', 'grosso_fiume', 'rrr_Groesseres_Fliessgewaesser', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5883,5883,'large_midland_creek','Grosser_Mittellandbach','gros_ruisseau_du_Plateau', 'grande_corso_d_acqua_dell_altopiano', 'rrr_Grosser_Mittellandbach', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8491,8491,'very_large_river','Grosses_Fliessgewaesser','tres_gros_cours_d_eau', 'fiume_molto_grande', 'rrr_Grosses_Fliessgewaesser', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5888,5888,'spring_waters','Quellgewaesser','region_de_source', 'acque_sorgive', 'rrr_Quellgewaesser', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (9149,9149,'lake','See','lac', 'lago', 'rrr_See', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.bio_ecol_assessment_type_water_body (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5890,5890,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.bio_ecol_assessment ADD CONSTRAINT fkey_vl_bio_ecol_assessment_type_water_body FOREIGN KEY (type_water_body)
 REFERENCES qgep_vl.bio_ecol_assessment_type_water_body (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
ALTER TABLE qgep_od.hydraulic_char_data ADD COLUMN fk_primary_direction varchar(16);
ALTER TABLE qgep_od.hydraulic_char_data ADD CONSTRAINT rel_hydraulic_char_data_primary_direction FOREIGN KEY (fk_primary_direction) REFERENCES qgep_od.wastewater_node(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_vl.hydraulic_char_data_is_overflowing ADD CONSTRAINT pkey_qgep_vl_hydraulic_char_data_is_overflowing_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.hydraulic_char_data_is_overflowing (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5774,5774,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_is_overflowing (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5775,5775,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_is_overflowing (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5778,5778,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.hydraulic_char_data_main_weir_kind ADD CONSTRAINT pkey_qgep_vl_hydraulic_char_data_main_weir_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.hydraulic_char_data_main_weir_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6422,6422,'leapingweir','Leapingwehr','LEAPING_WEIR', 'leaping_weir', 'rrr_Leapingwehr', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_main_weir_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6420,6420,'prank_weir_raised','Streichwehr_hochgezogen','deversoir_lateral_a_seuil_sureleve', 'stramazzo_laterale_alto', 'rrr_Streichwehr_hochgezogen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_main_weir_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6421,6421,'prank_weir_low','Streichwehr_niedrig','deversoir_lateral_a_seuil_abaisse', 'stamazzo_laterale_basso', 'rrr_Streichwehr_niedrig', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.hydraulic_char_data_pump_characteristics ADD CONSTRAINT pkey_qgep_vl_hydraulic_char_data_pump_characteristics_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.hydraulic_char_data_pump_characteristics (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6374,6374,'alternating','alternierend','alterne', 'alternato', 'rrr_alternierend', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_pump_characteristics (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6375,6375,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.hydraulic_char_data_pump_characteristics (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6377,6377,'parallel','parallel','parallele', 'parallelo', 'rrr_parallel', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_pump_characteristics (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6378,6378,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.hydraulic_char_data_status ADD CONSTRAINT pkey_qgep_vl_hydraulic_char_data_status_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.hydraulic_char_data_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6371,6371,'planned','geplant','prevu', 'pianificato', 'rrr_geplant', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6372,6372,'current','Ist','actuel', 'attuale', 'rrr_Ist', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.hydraulic_char_data_status (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6373,6373,'current_optimized','Ist_optimiert','actuel_opt', 'attuale_ottimizzato', 'rrr_Ist_optimiert', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.backflow_prevention_kind ADD CONSTRAINT pkey_qgep_vl_backflow_prevention_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.backflow_prevention_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5760,5760,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.backflow_prevention_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5757,5757,'backflow_flap','Rueckstauklappe','clapet_de_non_retour_a_battant', 'clappa_anti_rigurgito', 'clapeta _antirefulare', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.backflow_prevention_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5758,5758,'gate_shield','Stauschild','plaque_de_retenue', 'paratoia_cilindrica', 'rrr_Stauschild', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.backflow_prevention_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8636,8636,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.solids_retention_type ADD CONSTRAINT pkey_qgep_vl_solids_retention_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5664,5664,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8624,8624,'brush_rakes','Buerstenrechen','degrilleur_a_brosses', 'Ranghinatori_a_spazzola', 'rrr_Buerstenrechen', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5666,5666,'coarse_screen','Grobrechen','grille_grossiere', 'griglia_grossa', 'rrr_Grobrechen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5667,5667,'sieve','Sieb','tamis', 'filtro', 'rrr_Sieb', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8625,8625,'gate_shield','Stauschild','plaque_de_retenue', 'paratoia_cilindrica', 'rrr_Stauschild', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5668,5668,'scumboard','Tauchwand','paroi_plongeante', 'parete_sommersa', 'rrr_Tauchwand', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.solids_retention_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5669,5669,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.tank_cleaning_type ADD CONSTRAINT pkey_qgep_vl_tank_cleaning_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5621,5621,'airjet','Air_Jet','aeration_et_brassage', 'airjet', 'rrr_Air_Jet', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5620,5620,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8621,8621,'scraper_installation','Raeumereinrichtung','dispositif_de_curage', 'zzz_Raeumereinrichtung', 'rrr_Raeumereinrichtung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8622,8622,'agitator','Ruehrwerk','agitateur', 'agitatore', 'rrr_Ruehrwerk', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8623,8623,'meandered_gutter','Schlaengelrinne','cunette_courbe', 'cunetta_curva', 'rrr_Schlaengelrinne', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5623,5623,'surge_flushing','Schwallspuelung','rincage_en_cascade', 'lavaggio_getto', 'rrr_Schwallspülung', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5624,5624,'tipping_bucket','Spuelkippe','bac_de_rincage', 'benna_ribaltabile', 'rrr_Spuelkippe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_cleaning_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8626,8626,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.tank_emptying_type ADD CONSTRAINT pkey_qgep_vl_tank_emptying_type_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.tank_emptying_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5626,5626,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_emptying_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8638,8638,'gravitation','Gravitation','gravitation', 'gravitazione', 'rrr_Gravitation', '', '', '', '', '', 'true');
 ...
 INSERT INTO qgep_vl.tank_emptying_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5629,5629,'valve','Schieber','vanne', 'saracinesca', 'rrr_Schieber', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.tank_emptying_type (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8637,8637,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ...
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN fk_discharge_point varchar(16);
ALTER TABLE qgep_od.catchement_area_totals ADD CONSTRAINT rel_catchement_area_totals_discharge_point FOREIGN KEY (fk_discharge_point) REFERENCES qgep_od.discharge_point(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.catchement_area_totals ADD COLUMN fk_hydraulic_char_data varchar(16);
ALTER TABLE qgep_od.catchement_area_totals ADD CONSTRAINT rel_catchement_area_totals_hydraulic_char_data FOREIGN KEY (fk_hydraulic_char_data) REFERENCES qgep_od.hydraulic_char_data(obj_id) ON UPDATE CASCADE ON DELETE set null;
...
ALTER TABLE qgep_od.disposal ADD COLUMN fk_infiltration_installation varchar(16);
ALTER TABLE qgep_od.disposal ADD CONSTRAINT rel_disposal_infiltration_installation FOREIGN KEY (fk_infiltration_installation) REFERENCES qgep_od.infiltration_installation(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.disposal ADD COLUMN fk_discharge_point varchar(16);
ALTER TABLE qgep_od.disposal ADD CONSTRAINT rel_disposal_discharge_point FOREIGN KEY (fk_discharge_point) REFERENCES qgep_od.discharge_point(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.disposal ADD COLUMN fk_wastewater_structure varchar(16);
ALTER TABLE qgep_od.disposal ADD CONSTRAINT rel_disposal_wastewater_structure FOREIGN KEY (fk_wastewater_structure) REFERENCES qgep_od.wastewater_structure(obj_id) ON UPDATE CASCADE ON DELETE set null;
CREATE TABLE qgep_vl.disposal_disposal_place_current () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.disposal_disposal_place_current ADD CONSTRAINT pkey_qgep_vl_disposal_disposal_place_current_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4949,4949,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4946,4946,'liquid_manure_application','Guelleaustrag','epandage', 'zzz_Guelleaustrag', 'rrr_Guelleaustrag', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6474,6474,'none','keiner','aucun', 'nessuno', 'inexistent', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4947,4947,'public_sewer','oeffentlicheKanalisation','canalisation_publique', 'zzz_oeffentlicheKanalisation', 'rrr_oeffentlicheKanalisation', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4945,4945,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_current (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4948,4948,'central_WWTP','zentraleARA','STEP_centrale', 'zzz_zentraleARA', 'rrr_zentraleARA', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.disposal ADD CONSTRAINT fkey_vl_disposal_disposal_place_current FOREIGN KEY (disposal_place_current)
 REFERENCES qgep_vl.disposal_disposal_place_current (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.disposal_disposal_place_planned () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.disposal_disposal_place_planned ADD CONSTRAINT pkey_qgep_vl_disposal_disposal_place_planned_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4954,4954,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4951,4951,'liquid_manure_application','Guelleaustrag','epandage', 'zzz_Guelleaustrag', 'rrr_Guelleaustrag', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6400,6400,'none','keiner','aucun', 'nessuno', 'inexistent', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4952,4952,'public_sewer','oeffentlicheKanalisation','canalisation_publique', 'zzz_oeffentlicheKanalisation', 'rrr_oeffentlicheKanalisation', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4950,4950,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.disposal_disposal_place_planned (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4953,4953,'central_WWTP','zentraleARA','STEP_centrale', 'zzz_zentraleARA', 'rrr_zentraleARA', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.disposal ADD CONSTRAINT fkey_vl_disposal_disposal_place_planned FOREIGN KEY (disposal_place_planned)
 REFERENCES qgep_vl.disposal_disposal_place_planned (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.building_group ADD COLUMN fk_disposal varchar(16);
ALTER TABLE qgep_od.building_group ADD CONSTRAINT rel_building_group_disposal FOREIGN KEY (fk_disposal) REFERENCES qgep_od.disposal(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.building_group ADD COLUMN fk_measure varchar(16);
ALTER TABLE qgep_od.building_group ADD CONSTRAINT rel_building_group_measure FOREIGN KEY (fk_measure) REFERENCES qgep_od.measure(obj_id) ON UPDATE CASCADE ON DELETE set null;
CREATE TABLE qgep_vl.building_group_connecting_obligation () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_connecting_obligation ADD CONSTRAINT pkey_qgep_vl_building_group_connecting_obligation_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_connecting_obligation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5484,5484,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_connecting_obligation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5485,5485,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_connecting_obligation (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5486,5486,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_connecting_obligation FOREIGN KEY (connecting_obligation)
 REFERENCES qgep_vl.building_group_connecting_obligation (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_connection_wwtp () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_connection_wwtp ADD CONSTRAINT pkey_qgep_vl_building_group_connection_wwtp_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_connection_wwtp (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5095,5095,'connected','angeschlossen','raccorde', 'zzz_angeschlossen', 'rrr_angeschlossen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_connection_wwtp (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5096,5096,'not_connected','nicht_angeschlossen','non_raccorde', 'non_collegato', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_connection_wwtp (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5097,5097,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_connection_wwtp FOREIGN KEY (connection_wwtp)
 REFERENCES qgep_vl.building_group_connection_wwtp (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_drainage_map () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_drainage_map ADD CONSTRAINT pkey_qgep_vl_building_group_drainage_map_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_drainage_map (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4840,4840,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drainage_map (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4841,4841,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drainage_map (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4839,4839,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_drainage_map FOREIGN KEY (drainage_map)
 REFERENCES qgep_vl.building_group_drainage_map (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_drinking_water_network () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_drinking_water_network ADD CONSTRAINT pkey_qgep_vl_building_group_drinking_water_network_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_drinking_water_network (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4826,4826,'connected','angeschlossen','raccorde', 'zzz_angeschlossen', 'rrr_angeschlossen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_network (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4827,4827,'not_connected','nicht_angeschlossen','non_raccorde', 'non_collegato', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_network (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4825,4825,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_drinking_water_network FOREIGN KEY (drinking_water_network)
 REFERENCES qgep_vl.building_group_drinking_water_network (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_drinking_water_others () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_drinking_water_others ADD CONSTRAINT pkey_qgep_vl_building_group_drinking_water_others_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_drinking_water_others (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4833,4833,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_others (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4830,4830,'none','keine','aucune', 'nessuno', 'inexistent', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_others (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4831,4831,'source','Quelle','source', 'zzz_Quelle', 'rrr_Quelle', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_others (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4829,4829,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_drinking_water_others (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4832,4832,'cistern','Zisterne','citerne', 'zzz_Zisterne', 'rrr_Zisterne', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_drinking_water_others FOREIGN KEY (drinking_water_others)
 REFERENCES qgep_vl.building_group_drinking_water_others (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_electric_connection () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_electric_connection ADD CONSTRAINT pkey_qgep_vl_building_group_electric_connection_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_electric_connection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4836,4836,'connected','angeschlossen','raccorde', 'zzz_angeschlossen', 'rrr_angeschlossen', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_electric_connection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4837,4837,'not_connected','nicht_angeschlossen','non_raccorde', 'non_collegato', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_electric_connection (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4835,4835,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_electric_connection FOREIGN KEY (electric_connection)
 REFERENCES qgep_vl.building_group_electric_connection (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_function () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_function ADD CONSTRAINT pkey_qgep_vl_building_group_function_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4823,4823,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4820,4820,'holiday_building','Feriengebaeude','uniquement_vacances', 'zzz_Feriengebaeude', 'rrr_Feriengebaeude', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4821,4821,'industry_craft','IndustrieGewerbe','entreprise', 'zzz_IndustrieGewerbe', 'rrr_IndustrieGewerbe', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4822,4822,'farm','Landwirtschaftsbetrieb','exploitation_agricole', 'zzz_Landwirtschaftsbetrieb', 'rrr_Landwirtschaftsbetrieb', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4818,4818,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4819,4819,'residential_building','Wohngebaeude','uniquement_habitation', 'zzz_Wohngebaeude', 'rrr_Wohngebaeude', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_function FOREIGN KEY (function)
 REFERENCES qgep_vl.building_group_function (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.building_group_renovation_necessity () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.building_group_renovation_necessity ADD CONSTRAINT pkey_qgep_vl_building_group_renovation_necessity_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.building_group_renovation_necessity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8797,8797,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_renovation_necessity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8798,8798,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.building_group_renovation_necessity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (8799,8799,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.building_group ADD CONSTRAINT fkey_vl_building_group_renovation_necessity FOREIGN KEY (renovation_necessity)
 REFERENCES qgep_vl.building_group_renovation_necessity (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.building_group_baugwr ADD COLUMN fk_building_group varchar(16);
ALTER TABLE qgep_od.building_group_baugwr ADD CONSTRAINT rel_building_group_baugwr_building_group FOREIGN KEY (fk_building_group) REFERENCES qgep_od.building_group(obj_id) ON UPDATE CASCADE ON DELETE set null;
ALTER TABLE qgep_od.farm ADD COLUMN fk_building_group varchar(16);
ALTER TABLE qgep_od.farm ADD CONSTRAINT rel_farm_building_group FOREIGN KEY (fk_building_group) REFERENCES qgep_od.building_group(obj_id) ON UPDATE CASCADE ON DELETE cascade;
CREATE TABLE qgep_vl.farm_cesspit_volume () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.farm_cesspit_volume ADD CONSTRAINT pkey_qgep_vl_farm_cesspit_volume_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.farm_cesspit_volume (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5490,5490,'own_and_third_part_operation','Eigen_und_Fremdbetrieb','exploitation_propre_et_externe', 'zzz_Eigen_und_Fremdbetrieb', 'rrr_Eigen_und_Fremdbetrieb', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_cesspit_volume (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5488,5488,'own_operation','Eigenbetrieb','exploitation_propre', 'zzz_Eigenbetrieb', 'rrr_Eigenbetrieb', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_cesspit_volume (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5489,5489,'third_party_operation','Fremdbetrieb','exploitation_externe', 'zzz_Fremdbetrieb', 'rrr_Fremdbetrieb', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_cesspit_volume (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5491,5491,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.farm ADD CONSTRAINT fkey_vl_farm_cesspit_volume FOREIGN KEY (cesspit_volume)
 REFERENCES qgep_vl.farm_cesspit_volume (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.farm_conformity () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.farm_conformity ADD CONSTRAINT pkey_qgep_vl_farm_conformity_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.farm_conformity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4896,4896,'conform','konform','conforme', 'zzz_konform', 'rrr_konform', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_conformity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4898,4898,'restoration_postponed','Sanierung_aufgeschoben','differee', 'zzz_Sanierung_aufgeschoben', 'rrr_Sanierung_aufgeschoben', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_conformity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4897,4897,'restoration_pending','Sanierung_bevorstehend','imminente', 'zzz_Sanierung_bevorstehend', 'rrr_Sanierung_bevorstehend', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_conformity (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4895,4895,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.farm ADD CONSTRAINT fkey_vl_farm_conformity FOREIGN KEY (conformity)
 REFERENCES qgep_vl.farm_conformity (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.farm_continuance () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.farm_continuance ADD CONSTRAINT pkey_qgep_vl_farm_continuance_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.farm_continuance (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4890,4890,'not_defined','nicht_definiert','non_definie', 'zzz_nicht_definiert', 'rrr_nicht_definiert', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_continuance (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4892,4892,'improble','unwahrscheinlich','improbable', 'zzz_unwahrscheinlich', 'rrr_unwahrscheinlich', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_continuance (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4891,4891,'probable','wahrscheinlich','probable', 'zzz_wahrscheinlich', 'rrr_wahrscheinlich', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.farm ADD CONSTRAINT fkey_vl_farm_continuance FOREIGN KEY (continuance)
 REFERENCES qgep_vl.farm_continuance (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.farm_shepherds_hut_wastewater () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.farm_shepherds_hut_wastewater ADD CONSTRAINT pkey_qgep_vl_farm_shepherds_hut_wastewater_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.farm_shepherds_hut_wastewater (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4869,4869,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_shepherds_hut_wastewater (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4870,4870,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_shepherds_hut_wastewater (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4871,4871,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.farm ADD CONSTRAINT fkey_vl_farm_shepherds_hut_wastewater FOREIGN KEY (shepherds_hut_wastewater)
 REFERENCES qgep_vl.farm_shepherds_hut_wastewater (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.farm_stable_cattle () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.farm_stable_cattle ADD CONSTRAINT pkey_qgep_vl_farm_stable_cattle_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.farm_stable_cattle (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4875,4875,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_stable_cattle (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4876,4876,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.farm_stable_cattle (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (4877,4877,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.farm ADD CONSTRAINT fkey_vl_farm_stable_cattle FOREIGN KEY (stable_cattle)
 REFERENCES qgep_vl.farm_stable_cattle (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.small_treatment_plant ADD CONSTRAINT oorel_od_small_treatment_plant_wastewater_structure FOREIGN KEY (obj_id) REFERENCES qgep_od.wastewater_structure(obj_id) ON DELETE CASCADE ON UPDATE CASCADE;
CREATE TABLE qgep_vl.small_treatment_plant_function () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.small_treatment_plant_function ADD CONSTRAINT pkey_qgep_vl_small_treatment_plant_function_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5013,5013,'other','andere','autres', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5014,5014,'activated_sludge_process','Belebtschlammverfahren','boues_activees', 'zzz_Belebtschlammverfahren', 'rrr_Belebtschlammverfahren', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5022,5022,'bed_process','Bettverfahren','lit_bacterien', 'zzz_Bettverfahren', 'rrr_Bettverfahren', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5023,5023,'membran_bioreactor','Membranbioreaktor','reacteur_a_biomasse_fixee', 'zzz_Membranbioreaktor', 'rrr_Membranbioreaktor', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5020,5020,'constructed_wetland','Pflanzenklaeranlage','filtration_par_plantes', 'zzz_Pflanzenklaeranlage', '', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5019,5019,'sandfilter','Sandfilter','filtre_a_sable', 'zzz_Sandfilter', 'rrr_Sandfilter', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5015,5015,'sequencing_batch_reactor','SequencingBatchReactor','charge_sequentielle_SBR', 'zzz_SequencingBatchReactor', 'rrr_SequencingBatchReactor', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5016,5016,'immersion_trickle_filter','Tauchkoerper','disques_biologiques', 'zzz_Tauchkoerper', 'rrr_Tauchkoerper', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_function (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (5021,5021,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.small_treatment_plant ADD CONSTRAINT fkey_vl_small_treatment_plant_function FOREIGN KEY (function)
 REFERENCES qgep_vl.small_treatment_plant_function (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
CREATE TABLE qgep_vl.small_treatment_plant_remote_monitoring () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.small_treatment_plant_remote_monitoring ADD CONSTRAINT pkey_qgep_vl_small_treatment_plant_remote_monitoring_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.small_treatment_plant_remote_monitoring (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6414,6414,'yes','ja','oui', 'si', 'da', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_remote_monitoring (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6415,6415,'no','nein','non', 'no', 'nu', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.small_treatment_plant_remote_monitoring (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6416,6416,'unknown','unbekannt','inconnu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.small_treatment_plant ADD CONSTRAINT fkey_vl_small_treatment_plant_remote_monitoring FOREIGN KEY (remote_monitoring)
 REFERENCES qgep_vl.small_treatment_plant_remote_monitoring (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE qgep_od.drainless_toilet ADD CONSTRAINT oorel_od_drainless_toilet_wastewater_structure FOREIGN KEY (obj_id) REFERENCES qgep_od.wastewater_structure(obj_id) ON DELETE CASCADE ON UPDATE CASCADE;
CREATE TABLE qgep_vl.drainless_toilet_kind () INHERITS (qgep_sys.value_list_base);
--- neu qgep_vl statt qgep_od
ALTER TABLE qgep_vl.drainless_toilet_kind ADD CONSTRAINT pkey_qgep_vl_drainless_toilet_kind_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.drainless_toilet_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6411,6411,'other','andere','autre', 'altro', 'rrr_altul', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainless_toilet_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6408,6408,'chemical_toilet','chemischeToilette','toilette_chimique', 'toilette_chimica', 'rrr_chemischeToilette', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainless_toilet_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6410,6410,'compost_toilet','Komposttoilette','toilette_a_compost', 'toilette_compostante', 'rrr_Komposttoilette', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainless_toilet_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6412,6412,'unknown','unbekannt','inconu', 'sconosciuto', 'necunoscuta', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.drainless_toilet_kind (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (6409,6409,'incinerating_toilet','Verbrennungstoilette','toilette_d_incineration', 'toilette_inceneriscono', 'rrr_Verbrennungstoilette', '', '', '', '', '', 'true');
 ALTER TABLE qgep_od.drainless_toilet ADD CONSTRAINT fkey_vl_drainless_toilet_kind FOREIGN KEY (kind)
 REFERENCES qgep_vl.drainless_toilet_kind (code) MATCH SIMPLE 
 ON UPDATE RESTRICT ON DELETE RESTRICT;
...
COMMENT ON COLUMN qgep_od.wastewater_structure_text.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.wastewater_structure_text ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.wastewater_structure_text.classname IS 'Name of class that textclass is related to / Name der Klasse zu der die Textklasse gehört / nom de la classe à laquelle appartient la classe de texte';
...
ALTER TABLE qgep_od.wastewater_structure_text ADD COLUMN textpos_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_wastewater_structure_text_textpos_geometry ON qgep_od.wastewater_structure_text USING gist (textpos_geometry );
...
COMMENT ON COLUMN qgep_od.reach_text.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.reach_text ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.reach_text.classname IS 'Name of class that textclass is related to / Name der Klasse zu der die Textklasse gehört / nom de la classe à laquelle appartient la classe de texte';
...
ALTER TABLE qgep_od.reach_text ADD COLUMN textpos_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_reach_text_textpos_geometry ON qgep_od.reach_text USING gist (textpos_geometry );
...
COMMENT ON COLUMN qgep_od.catchment_area_text.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.catchment_area_text ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.catchment_area_text.classname IS 'Name of class that textclass is related to / Name der Klasse zu der die Textklasse gehört / nom de la classe à laquelle appartient la classe de texte';
...
ALTER TABLE qgep_od.catchment_area_text ADD COLUMN textpos_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_catchment_area_text_textpos_geometry ON qgep_od.catchment_area_text USING gist (textpos_geometry );
...
COMMENT ON COLUMN qgep_od.wastewater_structure_symbol.obj_id IS 'INTERLIS STANDARD OID (with Postfix/Präfix), see www.interlis.ch';
 ALTER TABLE qgep_od.wastewater_structure_symbol ADD COLUMN classname  varchar(50) ;
COMMENT ON COLUMN qgep_od.wastewater_structure_symbol.classname IS 'Name of class that symbol class is related to / Name der Klasse zu der die Symbolklasse gehört / nom de la classe à laquelle appartient la classe de symbole';
...
COMMENT ON COLUMN qgep_od.wastewater_structure_symbol.symbolori IS 'Default: 90 Degree / Default: 90 Grad / Default: 90 degree';
ALTER TABLE qgep_od.wastewater_structure_symbol ADD COLUMN symbolpos_geometry geometry('POINT', 2056);
CREATE INDEX in_qgep_wastewater_structure_symbol_symbolpos_geometry ON qgep_od.wastewater_structure_symbol USING gist (symbolpos_geometry );
...
ALTER TABLE qgep_vl.wastewater_structure_text_plantype ADD CONSTRAINT pkey_qgep_vl_wastewater_structure_text_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.wastewater_structure_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7844,7844,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7846,7846,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7847,7847,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7848,7848,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7845,7845,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.reach_text_plantype ADD CONSTRAINT pkey_qgep_vl_reach_text_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.reach_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7844,7844,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7846,7846,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7847,7847,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7848,7848,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.reach_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7845,7845,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.catchment_area_text_plantype ADD CONSTRAINT pkey_qgep_vl_catchment_area_text_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.catchment_area_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7844,7844,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7846,7846,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7847,7847,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7848,7848,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.catchment_area_text_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7845,7845,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_vl.wastewater_structure_symbol_plantype ADD CONSTRAINT pkey_qgep_vl_wastewater_structure_symbol_plantype_code PRIMARY KEY (code);
 INSERT INTO qgep_vl.wastewater_structure_symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7874,7874,'pipeline_registry','Leitungskataster','cadastre_des_conduites_souterraines', 'catasto_delle_canalizzazioni', 'rrr_Leitungskataster', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7876,7876,'overviewmap.om10','Uebersichtsplan.UeP10','plan_d_ensemble.pe10', 'piano_di_insieme.pi10', 'rrr_Uebersichtsplan.UeP10', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7877,7877,'overviewmap.om2','Uebersichtsplan.UeP2','plan_d_ensemble.pe2', 'piano_di_insieme.pi2', 'rrr_Uebersichtsplan.UeP2', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7878,7878,'overviewmap.om5','Uebersichtsplan.UeP5','plan_d_ensemble.pe5', 'piano_di_insieme.pi5', 'rrr_Uebersichtsplan.UeP5', '', '', '', '', '', 'true');
 INSERT INTO qgep_vl.wastewater_structure_symbol_plantype (code, vsacode, value_en, value_de, value_fr, value_it, value_ro, abbr_en, abbr_de, abbr_fr, abbr_it, abbr_ro, active) VALUES (7875,7875,'network_plan','Werkplan','plan_de_reseau', 'zzz_Werkplan', 'rrr_Werkplan', '', '', '', '', '', 'true');
...
ALTER TABLE qgep_od.measure ADD CONSTRAINT rel_od_measure_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.measure ADD CONSTRAINT rel_od_measure_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
...
ALTER TABLE qgep_od.waste_water_treatment_plant ADD CONSTRAINT rel_od_waste_water_treatment_plant_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.waste_water_treatment_plant ADD CONSTRAINT rel_od_waste_water_treatment_plant_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
...
ALTER TABLE qgep_od.log_card ADD CONSTRAINT rel_od_log_card_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.log_card ADD CONSTRAINT rel_od_log_card_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
...
ALTER TABLE qgep_od.catchement_area_totals ADD CONSTRAINT rel_od_catchement_area_totals_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.catchement_area_totals ADD CONSTRAINT rel_od_catchement_area_totals_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.disposal ADD CONSTRAINT rel_od_disposal_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.disposal ADD CONSTRAINT rel_od_disposal_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.building_group ADD CONSTRAINT rel_od_building_group_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.building_group ADD CONSTRAINT rel_od_building_group_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.building_group_baugwr ADD CONSTRAINT rel_od_building_group_baugwr_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.building_group_baugwr ADD CONSTRAINT rel_od_building_group_baugwr_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.farm ADD CONSTRAINT rel_od_farm_fk_dataowner FOREIGN KEY (fk_dataowner) REFERENCES qgep_od.organisation(obj_id);
ALTER TABLE qgep_od.farm ADD CONSTRAINT rel_od_farm_fk_dataprovider FOREIGN KEY (fk_provider) REFERENCES qgep_od.organisation(obj_id);
...
------ Indexes on identifiers
 ...
 CREATE UNIQUE INDEX in_od_measure_identifier ON qgep_od.measure USING btree (identifier ASC NULLS LAST, fk_dataowner ASC NULLS LAST);
 CREATE UNIQUE INDEX in_od_waste_water_treatment_plant_identifier ON qgep_od.waste_water_treatment_plant USING btree (identifier ASC NULLS LAST, fk_dataowner ASC NULLS LAST);
 ...
 CREATE UNIQUE INDEX in_od_catchement_area_totals_identifier ON qgep_od.catchement_area_totals USING btree (identifier ASC NULLS LAST, fk_dataowner ASC NULLS LAST);
 CREATE UNIQUE INDEX in_od_building_group_identifier ON qgep_od.building_group USING btree (identifier ASC NULLS LAST, fk_dataowner ASC NULLS LAST);
